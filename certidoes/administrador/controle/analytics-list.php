...
if($deleta<>'' and in_array('1', $excluir_permissao_s)){
	$query = $objQuery->SQLQuery("SELECT * FROM cp_analytics as an WHERE an.id_analytics ='".$id."'");
	$num = mysql_num_rows($query);
	if($num<>''){
		$res = mysql_fetch_array($query);
		$sql = $objQuery->SQLQuery("DELETE FROM cp_analytics WHERE id_analytics ='".$id."'");
	}
}
if($submit1){
	...
	for($i = 0; $i < count($id_anunc)-1; $i++){
		...
		if($errors!=1){
			$objQuery->SQLQuery("UPDATE cp_analytics SET st_id='".$alterar_status."' WHERE id_analytics='".$id_anunc[$i]."'");
			$id = $objQuery->ID;
			$done=1;
		}
	}
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
				...
				<form name="form" action="" method="GET" enctype="multipart/form-data">
					...
					<table width="100%" border="0">
						<tr>
							<td colspan="2"><label for="busca_dados" accesskey="1">Busca (ID / Unidade)</label></td>
			...
			<table width="1000" class="table_list">
				<tr>
					...
					<td width="1000" align="left" valign="middle">Unidade</td>
				...
				<?
				$onde ="";
				if($busca_dados<>''){$onde .= " and (an.id_analytics like '".$busca_dados."' or ue.fantasia like '".$busca_dados."%') ";}
				if($busca_status<>''){$onde .= " and an.st_id='".$busca_status."'";}
				$condicao = "FROM cp_analytics as an, cartorio_banco2.vsites_user_empresa as ue, cp_status as st WHERE an.id_empresa=ue.id_empresa AND an.st_id=st.st_id " .$onde. " ORDER BY an.id_analytics DESC";
				$campo = "ue.id_empresa, ue.fantasia, an.id_analytics, an.st_id, st.st_id, st.status, date_format(an.data, '%d/%m/%Y') as data";
				...
				while($res = mysql_fetch_array($query)){ 
					...
					<tr onmouseover="this.bgColor=\'#EEEEEE\';" onmouseout="this.bgColor=\'#FFFFFF\';">
						<td align="left" valign="middle"><input value="'.$res["id_analytics"].'" type="checkbox" name="checklister" id="checklister'.$i.'" title="Clique aqui para selecionar o registro" /></td>
						<td align="center" valign="middle">' . $res["id_analytics"] . '</td>
						...
						<td align="center" valign="middle"><a href="analytics-edit.php?id=' . $res["id_analytics"] . '" title="Clique aqui"><img src="../images/paginas/bt-editar.png" alt="bt-editar"/></a></td>';
						...
						<form name="form" action="" method="POST" enctype="multipart/form-data" onsubmit="return confirm(\'Deseja excluir este registro ?\');">
							<td align="center" valign="middle">
								<input type="hidden" name="id" value="'.$res["id_analytics"].'" />
