		...
		if($errors!=1){
			if(isset($_FILES['cat_imagem']) && $_FILES['cat_imagem']['name'] != null){
				...
				for($i = 0; $i < count($images); $i++){
					...
					$sql = "UPDATE cp_banners SET st_id='$st_id', nome_imagem='$nome_imagem', ";
					$sql .= "id_empresa='$id_empresa', destaque='$destaque',";
					$sql .= "cat_imagem='".$campo_img."' ";
					$sql .= "WHERE id_banners='$id'";
	...
	<div class="conteudo">
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
				...
				<?
				if($done!=1){
					$sql = $objQuery->SQLQuery("SELECT ps.id_banners, ps.st_id, ps.nome_imagem, ps.cat_imagem, ps.destaque, ue.id_empresa, st.st_id, st.status FROM cp_banners as ps, cp_status as st, cartorio_banco2.vsites_user_empresa as ue WHERE ps.id_empresa=ue.id_empresa AND ue.status='Ativo' AND ps.st_id=st.st_id AND ps.id_banners='" .$id. "'");
				...
				<form name="form" action="" method="post" enctype="multipart/form-data">
					...
					<table width="100%">
						...
						<tr>
							<td><label for="nome_imagem" accesskey="1">Nome do banner</label></td>
				...
				if($done){
					...
					echo '<meta HTTP-EQUIV="refresh" CONTENT="2; URL=banners-list.php" />';
				}
				?>
				</td>
				<tr>
