...
if($deleta<>'' and in_array('5', $excluir_permissao_s)){
	$query = $objQuery->SQLQuery("SELECT * FROM cp_cat_imagem as ci WHERE ci.id_cat_imagem ='".$id."'");
	$num = mysql_num_rows($query);
	if($num<>''){
		$res = mysql_fetch_array($query);
		$file_path = "../../upload/".$res['cat_imagem'];
		if(file_exists($file_path)) unlink($file_path);
		$sql = $objQuery->SQLQuery("DELETE FROM cp_cat_imagem WHERE id_cat_imagem ='".$id."'");
	}
}
if($submit1){
	...
	for($i = 0; $i < count($id_anunc)-1; $i++){
		...
		if($errors!=1){
			$objQuery->SQLQuery("UPDATE cp_cat_imagem SET st_id='".$alterar_status."' WHERE id_cat_imagem ='".$id_anunc[$i]."'");
			$id = $objQuery->ID;
			$done=1;
		}
	}
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
				...
				<td align="center" valign="top" class="td_info">
					...
					<table width="100%">
						...
						<?
						$permissao = verifica_permissao('editar_categoria_imagens',$controle_permissao_p,$controle_permissao_s);
						if($permissao == 'TRUE'){
						?>
			...
			<table width="1000" class="table_list">
				<tr>
					...
					<td width="239" align="left" valign="middle">Nome da categoria</td>
					<td width="239" align="left" valign="middle">Unidade</td>
					<td width="451" align="left" valign="middle">URL amigável</td>
				...
				<?
				$onde ="";
				if($busca_dados<>''){$onde .= " and (ci.id_cat_imagem like '".$busca_dados."' or ci.nome_imagem like '".$busca_dados."%') ";}
				if($busca_data<>''){$onde .= " and date_format(ci.data, '%d/%m/%Y') = '".$busca_data."' ";}
				if($busca_status<>''){$onde .= " and ci.st_id='".$busca_status."'";}
				$condicao = "FROM cp_cat_imagem as ci, cartorio_banco2.vsites_user_empresa as ue, cp_status as st WHERE ci.id_empresa=ue.id_empresa AND ci.st_id=st.st_id " .$onde. " ORDER BY ci.id_cat_imagem DESC";
				$campo = "ue.id_empresa, ue.fantasia, ci.id_cat_imagem, ci.nome_imagem, ci.url_amigavel, ci.cat_imagem, date_format(ci.data, '%d/%m/%Y') as data, st.st_id, st.status";
				...
				$query = $objQuery->paginacao( $campo , $condicao, $pagina , $url_busca, 2000000000000);
				$p_valor = "";
				$i = 0;
				while($res = mysql_fetch_array($query)){ 
					$p_valor .= '
					<tr onmouseover="this.bgColor=\'#EEEEEE\';" onmouseout="this.bgColor=\'#FFFFFF\';">
						<td align="left" valign="middle"><input value="'.$res["id_cat_imagem"].'" type="checkbox" name="checklister" id="checklister'.$i.'" title="Clique aqui para selecionar o registro" /></td>
						<td align="center" valign="middle">' . $res["id_cat_imagem"] . '</td>
						...
						<td align="center" valign="middle"><a href="categoria-imagem-edit.php?id=' . $res["id_cat_imagem"] . '" title="Clique aqui"><img src="../images/paginas/bt-editar.png" alt="bt-editar" /></a></td>';
						if(in_array('5', $excluir_permissao_s)){
						$p_valor .= '
						<form name="form" action="" method="POST" enctype="multipart/form-data" onsubmit="return confirm(\'Deseja excluir este registro ?\');">
							<td align="center" valign="middle">
								<input type="hidden" name="id" value="'.$res["id_cat_imagem"].'" />
