...
if($deleta<>'' and in_array('5', $excluir_permissao_s)){
	$query = $objQuery->SQLQuery("SELECT * FROM cp_imagem as im WHERE im.id_imagem ='".$id."'");
	$num = mysql_num_rows($query);
	if($num<>''){
		$res = mysql_fetch_array($query);
		$file_path = "../../upload/".$res['imagem'];
		if(file_exists($file_path)) unlink($file_path);
		$sql = $objQuery->SQLQuery("DELETE FROM cp_imagem WHERE id_imagem ='".$id."'");
	}
}
if($submit1){
	...
	for($i = 0; $i < count($id_anunc)-1; $i++){
		...
		if($errors!=1){
			$objQuery->SQLQuery("UPDATE cp_imagem SET st_id='".$alterar_status."' WHERE id_imagem='".$id_anunc[$i]."'");
			$id = $objQuery->ID;
			$done=1;
		}
	}
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
				...
				<td align="center" valign="top" class="td_info">
					...
					<table width="100%">
							...
							<td align="left" valign="top" class="td_info">
								<ul id="lista">
									<li>
										...
										if($permissao == 'TRUE'){
										?>
										<a href="categoria-imagem-add.php" title="Clique aqui" class="link_lista">Cadastro: Categoria de imagens</a>
										<?}?>
									</li>
									<li>
										<?
										...
										if($permissao == 'TRUE'){
										?>
										<a href="imagem-add.php" title="Clique aqui" class="link_lista">Cadastro: Imagens</a>
										<?}?>
									</li>
								</ul>
							</td>
						...
						<?
						$permissao = verifica_permissao('editar_imagens',$controle_permissao_p,$controle_permissao_s);
						if($permissao == 'TRUE'){
						?>
				...
				<form name="form" action="" method="GET" enctype="multipart/form-data">
					...
					<table width="100%" border="0">
						<tr>
							<td colspan="2"><label for="busca_dados" accesskey="1">Busca (ID / Nome da categoria / Unidade / Descrição da imagem)</label></td>
			...
			<table width="1000" class="table_list">
				<tr>
					...
					<td width="222" align="left" valign="middle">Nome da categoria</td>
					<td width="222" align="left" valign="middle">Unidade</td>
					<td width="484" align="left" valign="middle">Descrição da imagem</td>
					...
					<? if(in_array('5', $excluir_permissao_s)){ ?>
				...
				<?
				$onde ="";
				if($busca_dados<>''){$onde .= " and (im.id_imagem like '".$busca_dados."' or ci.nome_imagem like '".$busca_dados."%' or ci.descricao like '".$busca_dados."%' or ue.fantasia like '".$busca_dados."%') ";}
				if($busca_data<>''){$onde .= " and date_format(im.data, '%d/%m/%Y') = '".$busca_data."' ";}
				if($busca_status<>''){$onde .= " and im.st_id='".$busca_status."'";}
				$condicao = "FROM cp_cat_imagem as ci, cp_imagem as im, cartorio_banco2.vsites_user_empresa as ue, cp_status as st WHERE ci.id_cat_imagem=im.id_cat_imagem AND im.id_empresa=ue.id_empresa AND im.st_id=st.st_id " .$onde. " ORDER BY im.id_imagem DESC";
				$campo = "ci.id_cat_imagem, ci.nome_imagem, ci.descricao, ue.id_empresa, ue.fantasia, im.id_imagem, im.imagem, date_format(im.data, '%d/%m/%Y') as data, st.st_id, st.status";
				...
				$query = $objQuery->paginacao( $campo , $condicao, $pagina , $url_busca, 2000000000000000);
				$p_valor = "";
				$i = 0;
				while($res = mysql_fetch_array($query)){ 
					$p_valor .= '
					<tr onmouseover="this.bgColor=\'#EEEEEE\';" onmouseout="this.bgColor=\'#FFFFFF\';">
						<td align="left" valign="middle"><input value="'.$res["id_imagem"].'" type="checkbox" name="checklister" id="checklister'.$i.'" title="Clique aqui para selecionar o registro" /></td>
						<td align="center" valign="middle">' . $res["id_imagem"] . '</td>
						<td align="left" valign="middle">' . $res["nome_imagem"] . '</td>
						<td align="left" valign="middle">' . str_replace('Cartório Postal - ','',$res['fantasia']) . '</td>
						<td align="left" valign="middle">' . $res["descricao"] . '</td>
						<td align="center">';
						if($res['imagem']==""){
						...
						<td align="center" valign="middle"><a href="imagem-edit.php?id=' . $res["id_imagem"] . '" title="Clique aqui"><img src="../images/paginas/bt-editar.png" alt="bt-editar" /></a></td>';
						if(in_array('5', $excluir_permissao_s)){
						$p_valor .= '
						<form name="form" action="" method="POST" enctype="multipart/form-data" onsubmit="return confirm(\'Deseja excluir este registro ?\');">
							<td align="center" valign="middle">
								<input type="hidden" name="id" value="'.$res["id_imagem"].'" />
