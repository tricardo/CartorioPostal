...
if($deleta<>'' and in_array('12', $excluir_permissao_s)){
	$query = $objQuery->SQLQuery("SELECT * FROM cp_parcerias as ps WHERE ps.id_parcerias ='".$id."'");
	$num = mysql_num_rows($query);
	if($num<>''){
		$res = mysql_fetch_array($query);
		$file_path = "../../upload/".$res['cat_imagem'];
		if(file_exists($file_path)) unlink($file_path);
		$sql = $objQuery->SQLQuery("DELETE FROM cp_parcerias WHERE id_parcerias ='".$id."'");
	}
}
if($submit1){
	...
	for($i = 0; $i < count($id_anunc)-1; $i++){
		...
		if($errors!=1){
			$objQuery->SQLQuery("UPDATE cp_parcerias SET st_id='".$alterar_status."' WHERE id_parcerias ='".$id_anunc[$i]."'");
			$id = $objQuery->ID;
			$done=1;
		}
	}
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
							...
							<td align="left" valign="top" class="td_info">
								<ul id="lista">
									<li>
										...
										if($permissao == 'TRUE'){
										?>
										<a href="produtos-add.php" title="Clique aqui" class="link_lista">Cadastro: Produtos</a>
										<?}?>
									</li>
									<li>
										<?
										...
										if($permissao == 'TRUE'){
										?>
										<a href="parcerias-add.php" title="Clique aqui" class="link_lista">Cadastro: Parcerias</a>
										<?}?>
									</li>
									<li>
										<?
										...
										if($permissao == 'TRUE'){
										?>
										<a href="banners-add.php" title="Clique aqui" class="link_lista">Cadastro: Banners</a>
										<?}?>
									</li>
									<li>
										<?
										...
										if($permissao == 'TRUE'){
										?>
										<a href="analytics-add.php" title="Clique aqui" class="link_lista">Cadastro: Analytics</a>
										<?}?>
									</li>
								</ul>
							</td>
				...
				<form name="form" action="" method="GET" enctype="multipart/form-data">
					...
					<table width="100%" border="0">
						<tr>
							<td colspan="2"><label for="busca_dados" accesskey="1">Busca (ID / Nome da parceria / Unidade)</label></td>
			...
			<table width="1000" class="table_list">
				<tr>
					...
					<td width="32" align="center" valign="middle">ID</td>
					<td width="100" align="left" valign="middle">Unidade</td>
					<td width="239" align="left" valign="middle">Nome da parceria</td>
					...
					<? if(in_array('12', $excluir_permissao_s)){ ?>
				...
				<?
				$onde ="";
				if($busca_dados<>''){$onde .= " and (ps.id_parcerias like '".$busca_dados."' or ps.nome_imagem like '".$busca_dados."%' or ue.fantasia like '".$busca_dados."%') ";}
				if($busca_data<>''){$onde .= " and date_format(ps.data, '%d/%m/%Y') = '".$busca_data."' ";}
				if($busca_status<>''){$onde .= " and ps.st_id='".$busca_status."'";}
				$condicao = "FROM cp_parcerias as ps, cartorio_banco2.vsites_user_empresa as ue, cp_status as st WHERE ps.id_empresa=ue.id_empresa AND ps.st_id=st.st_id " .$onde. " ORDER BY ps.id_parcerias DESC";
				$campo = "ue.id_empresa, ue.fantasia, ps.id_parcerias, ps.nome_imagem, ps.cat_imagem, date_format(ps.data, '%d/%m/%Y') as data, st.st_id, st.status";
				...
				while($res = mysql_fetch_array($query)){ 
					...
					<tr onmouseover="this.bgColor=\'#EEEEEE\';" onmouseout="this.bgColor=\'#FFFFFF\';">
						<td align="left" valign="middle"><input value="'.$res["id_parcerias"].'" type="checkbox" name="checklister" id="checklister'.$i.'" title="Clique aqui para selecionar o registro" /></td>
						<td align="center" valign="middle">' . $res["id_parcerias"] . '</td>
						...
						$p_valor .= '
						<img src="../../upload/' . $res["cat_imagem"] . '" width="50" alt="" />';
						}
						$p_valor .= '
						</td>
						<td align="center" valign="middle">' . $res["data"] . '</td>
						<td align="center" valign="middle">' . $res["status"] . '</td>
						<td align="center" valign="middle"><a href="parcerias-edit.php?id=' . $res["id_parcerias"] . '" title="Clique aqui"><img src="../images/paginas/bt-editar.png" alt="bt-editar" /></a></td>';
						...
						<form name="form" action="" method="POST" enctype="multipart/form-data" onsubmit="return confirm(\'Deseja excluir este registro ?\');">
							<td align="center" valign="middle">
								<input type="hidden" name="id" value="'.$res["id_parcerias"].'" />
