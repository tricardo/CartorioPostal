		...
		if($errors!=1){
			if(isset($_FILES['cat_imagem']) && $_FILES['cat_imagem']['name'] != null){
				...
				for($i = 0; $i < count($images); $i++){
					...
					$sql = "UPDATE cp_produtos SET st_id='$st_id', nome_imagem='$nome_imagem', ";
	...
	<div class="conteudo">
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
				...
				<?
				if($done!=1){
					$sql = $objQuery->SQLQuery("SELECT ps.id_produtos, ps.st_id, ps.nome_imagem, ps.cat_imagem, ps.destaque, ue.id_empresa, st.st_id, st.status FROM cp_produtos as ps, cp_status as st, cartorio_banco2.vsites_user_empresa as ue WHERE ps.id_empresa=ue.id_empresa AND ue.status='Ativo' AND ps.st_id=st.st_id AND ps.id_produtos='" .$id. "'");
				...
				<form name="form" action="" method="post" enctype="multipart/form-data">
					...
					<table width="100%">
						...
						<tr>
							<td><label for="nome_imagem" accesskey="1">Nome do produto</label></td>
				...
				if($done){
					...
					echo '<meta HTTP-EQUIV="refresh" CONTENT="2; URL=produtos-list.php" />';
				}
				?>
				</td>
				<tr>
