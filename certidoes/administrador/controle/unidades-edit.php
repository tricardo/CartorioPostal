...
pt_register('POST','total');
	if($submit1){
		if($errors!=1){
			$objQuery->SQLQuery("UPDATE cp_unidades SET total='$total' WHERE id_unidades='$id'");
			$id = $objQuery->ID;
			$done=1;
		}
	}
	...
	<div class="conteudo">
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
				...
				<?
				if($done!=1){
					$sql = $objQuery->SQLQuery("SELECT * FROM cp_unidades as un WHERE un.id_unidades='" .$id. "'");
					$res = mysql_fetch_array($sql);
					$id_unidades	= $res['id_unidades'];
					$total			= $res['total'];
				?>
				...
				<form name="form" action="" method="post" enctype="multipart/form-data">
					...
					<table width="100%">
						<tr>
							<td><label for="total" accesskey="1">Total</label></td>
						</tr>
						<tr>
							<td><input name="total" type="text" id="total" tabindex="1" value="<?= $total?>" /></td>
						</tr>
						<tr>
				...
				if($done){
					...
					echo '<meta HTTP-EQUIV="refresh" CONTENT="2; URL=unidades-list.php" />';
				}
				?>
				</td>
				<tr>
