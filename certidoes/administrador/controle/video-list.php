...
if($deleta<>'' and in_array('6', $excluir_permissao_s)){
	$query = $objQuery->SQLQuery("SELECT * FROM cp_video as v WHERE v.id_video ='".$id."'");
	$num = mysql_num_rows($query);
	if($num<>''){
		$res = mysql_fetch_array($query);
		$sql = $objQuery->SQLQuery("DELETE FROM cp_video WHERE id_video ='".$id."'");
	}
}
if($submit1){
	...
	for($i = 0; $i < count($id_anunc)-1; $i++){
		...
		if($errors!=1){
			$objQuery->SQLQuery("UPDATE cp_video SET st_id='".$alterar_status."' WHERE id_video='".$id_anunc[$i]."'");
			$id = $objQuery->ID;
			$done=1;
		}
	}
		...
		<div class="pages">
			...
			<table width="1000" class="table_info">
				...
				<td align="center" valign="top" class="td_info">
					...
					<table width="100%">
							...
							<td align="left" valign="top" class="td_info">
								<ul id="lista">
									<li>
										...
										if($permissao == 'TRUE'){
										?>
										<a href="categoria-video-add.php" title="Clique aqui" class="link_lista">Cadastro: Categoria de vídeos</a>
										<?}?>
									</li>
									<li>
										<?
										...
										if($permissao == 'TRUE'){
										?>
										<a href="video-add.php" title="Clique aqui" class="link_lista">Cadastro: Vídeos</a>
										<?}?>
									</li>
								</ul>
							</td>
						...
						<?
						$permissao = verifica_permissao('editar_videos',$controle_permissao_p,$controle_permissao_s);
						if($permissao == 'TRUE'){
						?>
				...
				<form name="form" action="" method="GET" enctype="multipart/form-data">
					...
					<table width="100%" border="0">
						<tr>
							<td colspan="2"><label for="busca_dados" accesskey="1">Busca (ID / Nome da categoria / Descrição do vídeo)</label></td>
			...
			<table width="1000" class="table_list">
				<tr>
					...
					<td width="380" align="left" valign="middle">Nome da categoria</td>
					<td width="326" align="left" valign="middle">Descrição do vídeo</td>
				...
				<?
				$onde ="";
				if($busca_dados<>''){$onde .= " and (v.id_video like '".$busca_dados."' or cv.nome_video like '".$busca_dados."%' or v.descricao_video like '".$busca_dados."%') ";}
				if($busca_data<>''){$onde .= " and date_format(cv.data, '%d/%m/%Y') = '".$busca_data."' ";}
				if($busca_status<>''){$onde .= " and cv.st_id='".$busca_status."'";}
				$condicao = "FROM cp_cat_video as cv, cp_video as v, cp_status as st WHERE v.id_cat_video=cv.id_cat_video AND v.st_id=st.st_id " .$onde. " ORDER BY v.id_video DESC";
				$campo = "cv.id_cat_video, cv.nome_video, cv.imagem_video, v.id_video, v.descricao_video, date_format(v.data, '%d/%m/%Y') as data, v.st_id, st.st_id, st.status";
				...
				while($res = mysql_fetch_array($query)){ 
					...
					<tr onmouseover="this.bgColor=\'#EEEEEE\';" onmouseout="this.bgColor=\'#FFFFFF\';">
						<td align="left" valign="middle"><input value="'.$res["id_video"].'" type="checkbox" name="checklister" id="checklister'.$i.'" title="Clique aqui para selecionar o registro" /></td>
						<td align="center" valign="middle">' . $res["id_video"] . '</td>
						<td align="left" valign="middle">' . $res["nome_video"] . '</td>
						<td align="left" valign="middle">' . $res["descricao_video"] . '</td>
						<td align="center">';
						if($res['imagem_video']==""){
						...
						<td align="center" valign="middle"><a href="video-edit.php?id=' . $res["id_video"] . '" title="Clique aqui"><img src="../images/paginas/bt-editar.png" alt="bt-editar" /></a></td>';
						...
						<form name="form" action="" method="POST" enctype="multipart/form-data" onsubmit="return confirm(\'Deseja excluir este registro ?\');">
							<td align="center" valign="middle">
								<input type="hidden" name="id" value="'.$res["id_video"].'" />
