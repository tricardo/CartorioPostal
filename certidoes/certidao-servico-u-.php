	...
	*/
    ...
    if (count($errors) < 1) {
        if ($_SESSION['id_pedido'] == '') {
			...
			$rodizio = 1;
			#nao eh rodizio // vai direto pra franquia e nao conta rodizio
			if($_SERVER["REMOTE_ADDR"] == '189.112.169.1'){
				if((isset($_GET['estado']) AND isset($_GET['cidade']) AND isset($_GET['link'])) OR isset($_POST['estado'])){
					$Empresa = new $EmpresaDAO();
					$data = $EmpresaDAO->buscaempresa();
					if($data[0] == 1){
						$rodizio = 0;
						$controle_id_empresa = $data[1];
						$controle_id_usuario = $data[2];
			...
			if($_SERVER["REMOTE_ADDR"] == '200.158.92.1'){
				#print_r($_POST);
				#echo $rodizio;
				#print_r($data);
				#exit;
		...
		$msg .= "------------------------------------------------------------------------<br />";
		$msg .= "<strong>Parceria Uniliver: Cartório Postal:.</strong><br />";
		$msg .= "------------------------------------------------------------------------<br />";
		$msg .= "<strong>Nome ".$nome;
		$msg .= "<br />";
		$msg .= "<strong>Número da Ordem: ".$cadastrar_pedido;
		$formato = "\nContent-type: text/html\n charset=iso-8859-1\n";
		$destinatario = "parceria.unilever@cartoriopostal.com.br";
		#$destinatario = "antonio.alves@softfox.com.br";
		$titulo = "Novo Pedido - Parceria Uniliver";
        ...
        <script>
            if(confirm('Pedido enviado com sucesso!\nO número da ordem é: <?= $cadastrar_pedido ?>\n\nDeseja Solicitar outro serviço?')){
				window.location="http://www.cartoriopostal.com.br/certidoes/parceria-unilever/";
            }else{
                window.location="http://www.cartoriopostal.com.br/certidoes/parceria-unilever-certidao.php?p_novo=1";	
            }
        </script>
        <?
        exit;
