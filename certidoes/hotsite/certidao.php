...
if($submit_pedido <> '') {//check for errors
	...
	if(isset($_POST)){
		foreach($_POST as $cp => $valor){
			$$cp = char_upper(strtoupper(strtolower($valor)));
		}
	}
	...
	if($id_servico_departamento == '' and $id_servico_var <> ''){
		if ($errors != 1){
			$error.="<span style='font: 12px Arial; color:#000000;'>Erro ao cadastrar o pedido. Por favor entre em contato com nossa central de atendimento (11) 3103-0800</span><br />";
		}
		$errors['id_servico'] = 1;
	}
	...
	if(count($errors) < 1){
		if($_SESSION['id_pedido'] == ''){
			$controle_id_empresa = $fr->id_empresa;
			$controle_id_usuario = $fr->id_usuario;
			...
			if ($controle_id_empresa == '' OR $controle_id_usuario == ''){
		...
		<script src="http://www.googleadservices.com/pagead/conversion.js" language="javascript" type="text/javascript"></script>
		...
		<script language="javascript" type="text/javascript">
			if(confirm('Pedido enviado com sucesso!\nO número da ordem é: <?= $cadastrar_pedido;?>\n\nDeseja Solicitar outro serviço?')){
				window.location="/<?= $fr->id_empresa ?>/<?= $fr->link_estado ?>/<?= $fr->link_cidade;?>/certidao";
			}else{
				window.location="/<?= $fr->id_empresa ?>/<?= $fr->link_estado ?>/<?= $fr->link_cidade;?>/novo";
			}
		</script>
		<?
		exit;
...
?>
<?
$sql = $objQuery->SQLQuery("SELECT '".$fr->id_empresa."', an.id_analytics, an.st_id, an.descricao FROM cartorio_banco2.vsites_user_empresa as ue, cp_analytics as an WHERE '".$fr->id_empresa."'=an.id_empresa AND an.st_id='1' AND ue.id_empresa='".$fr->id_empresa."' ORDER BY an.id_analytics DESC");
...
<div id="area-container">
	<div id="container">
		<div id="destaque-imagens-paginas">
			<img src="<?= URL_IMAGES;?>hotsite/certidao.png" alt="Solicitante sua certidão" title="Solicitante sua certidão" />
		</div>
		<div class="box-01">
			<div class="nav">
				<div class="nav-02">
					<table width="100%" border="0" align="center" cellspacing="5" cellpadding="5" bgcolor="#FFFFFF">
						<tr>
							<td align="left" valign="middle" bgcolor="#FFFFFF" colspan="4">
								<h1 style="font-weight: bold; color: #202A72; text-transform: uppercase;">CERTIDÃO É NA CARTÓRIO POSTAL - <?= $fr->fantasia;?>:.</h1>
					...
					<form name="form_orcamento1" id="form_orcamento1" action="" method="post" enctype="multipart/form-data">
						<table width="100%" border="0" align="center" cellspacing="5" cellpadding="5" bgcolor="#FFFFFF">
							<tr>
								<td colspan="5">
									Solicite seu orçamento e em breve nossa equipe entrará em contato para informar os valores e confirmar sua solicitação!</p>
									...
									<?}else{?>
										<p><strong>Prezado(a) <?= $p->nome;?>, </strong><br />
											...
											<strong>Para solicitar um novo serviço, preencha os campos abaixo, caso contrário <a href="/<?= $fr->id_empresa ?>/<?= $fr->link_estado ?>/<?= $fr->link_cidade ?>/novo" title="Clique aqui" class="link-normal">clique aqui!</a></strong>
										</p>
							...
							<tr>
								...
								<td>
									...
									<select name="tipo" id="tipo" style="width: 70px;" <?= ($errors['tipo']) ? 'style="border: 1px solid #FF0000; width: 100%; height: 20px;"' : '';?> >
										...
										<?if($pais!='Brazil'){?>
											<option value="outro" <?if ($p->tipo == 'outro') echo 'selected="selected"';?>>OUTRO</option>
										<?}?>
									</select>
