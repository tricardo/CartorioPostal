...
if($permissao == 'FALSE' and $safpostal_id_usuario!="56" or $safpostal_id_empresa!='1'){ 
...
<td width="764" align="center" valign="top" background="../images/paginas/index/barra_de_titulo1.png">
	<table width="768" border="0" cellspacing="0" cellpadding="0">
	<tr>
	<td width="345" height="20" align="left" valign="middle"><strong>Mala Direta: Comunicado Interno</strong></td>
...
<td>
    <div id="conteudo">
			...
			if ($submit1){
				...
				$sql = $objQuery->SQLQuery("insert into saf_comunicado_int(data,id_news,id_empresa,departamento, assunto, texto, id_usuario) values(NOW(),'".$id_news."','".$id_empresa."','".$departamento."','".$assunto."','".$texto."','".$safpostal_id_usuario."')");
				$id_maladireta = $objQuery->ID;
				$Subject = 'CI '.$id_maladireta.' - '.$assunto;
				//error_reporting(0);
				set_time_limit(0);
				...
				if($erro==''){
					...
					ue.status='Ativo' and ue.id_empresa=uu.id_empresa and ue.id_empresa=1 and uu.status='Ativo' 
					".$onde);
					while($res = mysql_fetch_array($sql)){
						$Recipiant = $res['email']; 
        ...
        <?
		if ($done) {
		echo '<div class="respotas_sucesso"><br>MALA DIRETA ENVIADA COM SUCESSO!<br><a href="comunicado_emporium.php">Clique para voltar</a><br>
			  </div>';
		 }	  
...
if($done == ''){			
        ...
        <td>
			...
			<tr>
			<td height="50" colspan="2" align="center" valign="middle" bgcolor="#FFFFFF"><strong>MALA DIRETA DA FRANCHISE EMPORIUM</strong></td>
			</tr>
			...
			<?
			$sql = $objQuery->SQLQuery("SELECT id_empresa, fantasia FROM vsites_user_empresa as ue WHERE ue.id_empresa='1' ORDER BY fantasia");
