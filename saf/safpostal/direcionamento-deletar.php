...
<div id="principal">
	...
	<div style="width:768px;">
	  <div id="titulo">
			<div id="titulo1">&nbsp;&nbsp;Direcionamento</div>
      ...
      <? if($safpostal_id_usuario == 1 || $safpostal_id_usuario == 56 || $safpostal_id_usuario == 272){?>
        ...
        <table width="100%" border="0" cellspacing="2" cellpadding="0" style="float:left; color:#333;">
            	...
            	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td height="30" colspan="2">
                    	Direcionar: 
                        <select id="usuario" name="usuario" class="form_estilo" style="width:608px;">
                  ...
                  <tr>
                    <td width="50%" height="30">Consultor:
                      <? if($mostrar == 1){ ?>
                        ...
                        <? } ?>                    </td>
                    <td>Nome:
                      <input value="<?=$c->nome?>" type="text" name="nome" id="nome" class="form_estilo" style="width:333px; margin-left:4px;" maxlength="50" /></td>
                  </tr>
                  <tr>
                    <td height="30">Status:
                      <select name="id_status" id="id_status" class="form_estilo" style="width:320px;">
                          <option value="0"></option>
					...
					if(count($e) > 0){ ?>
                      Cidade:
                      <select name="cidade" id="cidade" class="form_estilo" style="width:266px; margin-left:-3px;">
          <option value=""></option>
          <? foreach($e as $j => $ci){?>
          <option value="<?=trim(strtoupper($ci->cidade_interesse))?>" <? echo (strtolower(trim($ci->cidade_interesse)) == strtolower(trim($c->cidade))) ? 'selected="selected"' : '';?>>
            <?=trim(strtoupper($ci->cidade_interesse))?>
            </option>
          <? }?>
        </select>
                      <? }
					...
					if(count($e) > 0){ ?>
                      UF:
                      <select name="uf" id="uf" class="form_estilo" style="width:40px;">
                        <option value=""></option>
                        <? foreach($e as $j => $uf){?>
                        <option value="<?=trim(strtoupper($uf->estado_interesse))?>" <? echo ($uf->estado_interesse == $c->uf) ? 'selected="selected"' : '';?>>
                          <?=trim(strtoupper($uf->estado_interesse))?>
                        </option>
                        <? }?>
                      </select>
                      <? } else { echo '&nbsp;'; }?>                    </td>
                  ...
                  <tr>
					...
					if(count($e) > 0){ ?>
                      Ano:
                      <select name="ano" id="ano" class="form_estilo" style="width:80px;">
                        <option value="0"></option>
                        <? foreach($e as $j => $a){?>
                        <option value="<?=$a->data?>" <? echo ($a->data == $c->ano) ? 'selected="selected"' : '';?>>
                          <?=$a->data?>
                        </option>
                        <? }?>
                      </select>
                      <? } else { echo '&nbsp;'; }?></td>
                    <td align="right"><input type="submit" value="Filtrar" name="btn2" id="btn2" class="form_estilo" 
                    style="width:80px; height:20px; text-align:center; vertical-align:middle;" /></td>
                  </tr>
                </table>
           	<label style="margin-left:10px; margin-right:10px; font-weight:bold;"></label></td>
          </tr>
          <? $dt = $ficha->site_ficha_cadastro($c); $i = 0;
		  if($dt[0][0]->Total > 0){?>
          <tr>
            <td colspan="6">Foram encontrado(s) <?=$dt[0][0]->Total?> registro(s).</td>
          </tr>          
          ...
          title="Clique na linha para visualizar este registro.">
            <td class="td2" style="text-align:center"><input type="checkbox" class="form_estilo" value="<?=$e->id_ficha?>" id="id_ficha<?=$i?>" name="id_ficha[]" /></td>
            <td onclick="javascript:location.href='interessados_edit.php?id=<?=$e->id_ficha?>'" class="td2" style="text-align:left; height:20px;">&nbsp;<?=ucwords(strtolower($e->nome))?></td>
            <td onclick="javascript:location.href='interessados_edit.php?id=<?=$e->id_ficha?>'" class="td2" style="text-align:center">&nbsp;<?=ucwords(strtolower($res['nome']))?>&nbsp;</td>
