...
if ($submit1){
	...
	pt_register('POST','id_ftp_categoria');
	...
	if($errors!=1){	
		...
		if($file1['name']<>''){
			...
			preg_match("/\.(jpeg|jpg|png|gif|bmp|csv|pdf|msword|zip|rar|doc|docx|xls|xlsx|ppt|pptx|ods|odt|odp|cdr){1}$/i", $file1["name"], $ext);
			...
			$query="INSERT INTO saf_ftp (id_usuario, id_empresa, id_ftp_categoria, arquivo, titulo, extensao, data) values ('".$safpostal_id_usuario."','".$id_empresa."','".$id_ftp_categoria."','".$file1_name."','".$titulo1."','".$ext[1]."',NOW())";
		...
		if($file2['name']){
			// Pega extensão do file_anexo
			preg_match("/\.(jpeg|jpg|png|gif|bmp|csv|pdf|msword|zip|rar|doc|docx|xls|xlsx|ppt|pptx|ods|odt|odp|cdr){1}$/i", $file2["name"], $ext);
			// Gera um nome único para a imagem
			...
			$query="INSERT INTO saf_ftp (id_usuario, id_empresa, id_ftp_categoria, arquivo, titulo, extensao, data) values ('".$safpostal_id_usuario."','".$id_empresa."','".$id_ftp_categoria."','".$file2_name."','".$titulo2."','".$ext[1]."',NOW())";
			$result = $objQuery->SQLQuery($query);
			$id = $objQuery->ID;				
		}	
		...
		if($file3['name']){
			// Pega extensão do file_anexo
			preg_match("/\.(jpeg|jpg|png|gif|bmp|csv|pdf|msword|zip|rar|doc|docx|xls|xlsx|ppt|pptx|ods|odt|odp|cdr){1}$/i", $file3["name"], $ext);
			// Gera um nome único para a imagem
			...
			$query="INSERT INTO saf_ftp (id_usuario, id_empresa, id_ftp_categoria, arquivo, titulo, extensao, data) values ('".$safpostal_id_usuario."','".$id_empresa."','".$id_ftp_categoria."','".$file3_name."','".$titulo3."','".$ext[1]."',NOW())";
			$result = $objQuery->SQLQuery($query);
			$id = $objQuery->ID;				
		}	
		...
		if($file4['name']){
			// Pega extensão do file_anexo
			preg_match("/\.(jpeg|jpg|png|gif|bmp|csv|pdf|msword|zip|rar|doc|docx|xls|xlsx|ppt|pptx|ods|odt|odp|cdr){1}$/i", $file4["name"], $ext);
			// Gera um nome único para a imagem
			...
			$query="INSERT INTO saf_ftp (id_usuario, id_empresa, id_ftp_categoria, arquivo, titulo, extensao, data) values ('".$safpostal_id_usuario."','".$id_empresa."','".$id_ftp_categoria."','".$file4_name."','".$titulo4."','".$ext[1]."',NOW())";
			$result = $objQuery->SQLQuery($query);
			$id = $objQuery->ID;				
		}		
		...
		if($file5['name']){
			// Pega extensão do file_anexo
			preg_match("/\.(jpeg|jpg|png|gif|bmp|csv|pdf|msword|zip|rar|doc|docx|xls|xlsx|ppt|pptx|ods|odt|odp|cdr){1}$/i", $file5["name"], $ext);
			// Gera um nome único para a imagem
			...
			$query="INSERT INTO saf_ftp (id_usuario, id_empresa, id_ftp_categoria, arquivo, titulo, extensao, data) values ('".$safpostal_id_usuario."','".$id_empresa."','".$id_ftp_categoria."','".$file5_name."','".$titulo5."','".$ext[1]."',NOW())";
			$result = $objQuery->SQLQuery($query);
			$id = $objQuery->ID;				
		}	
		$done=1;
...
?>
        ...
        <td colspan="3">
        <select name="id_empresa" style="width:358px">
        <option value="">Todas</option>
        <?
        $sql = $objQuery->SQLQuery("SELECT id_empresa, fantasia FROM vsites_user_empresa as ue WHERE status != 'Cancelado' ORDER BY fantasia ");
        ...
        <select name="id_ftp_categoria">
        	<?
        $sql = $objQuery->SQLQuery("SELECT * FROM  saf_ftp_categoria as C ORDER BY id_ftp_categoria");
		
		while($res = mysql_fetch_array($sql)){
			echo '<option value="'.$res['id_ftp_categoria'].'"';
			
				if($busca_status==$res['id_ftp_categoria']) echo 'selected="selected"'; 
		...
		echo '<div id="mensagem_ftp"><strong><span style="font-size:12px">Registro efetuado com sucesso!</span></strong><br><br>
			  <a href="ftp_list.php" title="Clique aqui"><span style="font-size:12px">Clique aqui para visualizar a lista de arquivos</span></a>
			  </div>';
		}	  
		?>
