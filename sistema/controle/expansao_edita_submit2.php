...
if($c->id_status == 0){
	...
	$cp     = 'id_status'; 
} elseif(($c->id_status == 5 || $c->id_status == 10 || $c->id_status == 12 || $c->id_status == 19) && $errors == 0){
		
	if($c->data_reuniao == '//' || $c->data_reuniao == '' && $errors == 0 && $c->id_status != 19){
		$errors++;
...
if($errors == 0){
	...
	if($c->id_status == 19 and $e->id_status != 1){
		$lista = array(
			'id_user_alt'=>$controle_id_usuario, 'observacao_expansao'=>$c->observacao_expansao, 
			'data_reuniao'=>$y.'-'.$m.'-'.$d, 'data_reuniao_inclusao'=>date ('Y-m-j') . ' ' . date ('H:i:s'), 
			'id_ficha'=>$id
		);
	} else {
		$lista = array(
			'id_user_alt'=>$controle_id_usuario,'id_usuario'=>$c->id_usuario,'id_status'=>$c->id_status, 'observacao_expansao'=>$c->observacao_expansao, 		
			'data_reuniao'=>$y.'-'.$m.'-'.$d, 'data_reuniao_inclusao'=>date ('Y-m-j') . ' ' . date ('H:i:s'), 
			'id_ficha'=>$id
		);	
	}
	...
	$lista = array(
		...
		'id_status'=>$id_status_anterior, 'data_reuniao'=>$y.'-'.$m.'-'.$d, 'data_inclusao'=>$data_reuniao_inclusao_anterior,
	...
	if($c->id_status != 20){		
		...
		WHERE uu.status='Ativo' AND uu.id_empresa=".$controle_id_empresa." AND uu.id_usuario = ".$c->id_usuario." ORDER BY uu.nome";
	
		$dt = $objQuery->SQLQuery($sql);
		$res = mysql_fetch_array($dt);
		
		switch($c->id_status){
			case 4: if($id_status_atual != 19){require "expansao_envia_email_elaborar_cof.php";} break;
			case 9: if($id_status_atual != 19){require "expansao_envia_email_emitir_contrato.php";} break;
			case 14: if($id_status_atual != 19){require "expansao_envia_email_finalizar.php";} break;
		}
	}
	//recarregar página
	echo "<script>location.href='expansao_interessados_edit.php?id=".$id."&aba=2';</script>";
	exit();
}
?>