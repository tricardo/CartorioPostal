...
if ($submit_form) {
    ...
    if ($id_conta > 2) {
        ...
        if ($instrucao1 == 6 and $instrucao2 < 5) {
            $errors['instrucao2'] = 1;
            $error .= '<li>O campo instrução 2 não pode ser menor que 5</li>';
        }
    }

    if (empty($ocorrencia)) {
        $errors['ocorrencia'] = 1;
        $result .= "<li>O campo <strong>Ocorrência</strong> é obrigatório.</li>";
    }
    ...
    if ($id_fatura <> '') {
        ...
        if ($verifica_fatura == 0 or $verifica_fatura == '') {
            $errors['id_fatura'] = 1;
            $result .= '<li>Número de Fatura inválido</li>';
        }
    }
