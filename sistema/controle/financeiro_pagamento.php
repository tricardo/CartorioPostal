...
if($busca_submit<>''){
	...
	pt_register('GET','busca_data_i_f');	
	pt_register('GET','busca_data_f_f');	
	...
	$_SESSION['f_busca_data_i_f'] 			= $busca_data_i_f;	
	$_SESSION['f_busca_data_f_f'] 			= $busca_data_f_f;	
	...
	$busca_data_i_f          	= $_SESSION['f_busca_data_i_f'];
	$busca_data_f_f          	= $_SESSION['f_busca_data_f_f'];
...
else $busca_data_i = date('2011-m-d');

if($busca_data_f <> '') $busca_data_f = invert($busca_data_f,'-','SQL'); 
else $busca_data_f = date('Y-m-d');

if($busca_data_i_f <> '') $busca_data_i_f = invert($busca_data_i_f,'-','SQL'); 
if($busca_data_f_f <> '') $busca_data_f_f = invert($busca_data_f_f,'-','SQL'); 

if($busca_data_ci <> '') $busca_data_ci = invert($busca_data_ci,'-','SQL');
if($busca_data_cf <> '') $busca_data_cf = invert($busca_data_cf,'-','SQL');
if($busca_data_i_a <> '') $busca_data_i_a = invert($busca_data_i_a,'-','SQL'); else $busca_data_i_a = date('2011-m-d');
...
$buscap->busca_data_i_f=$busca_data_i_f;
$buscap->busca_data_f_f=$busca_data_f_f;
...
<div class="busca1">	
	...
	<select name="busca_autorizacao" style="width: 200px; float: left" class="form_estilo">		
		...
		<option value="Recebido" <? if($busca_autorizacao=='Recebido') echo 'selected="select"'; ?>>Recebido</option>
                <option value="_" <? if($busca_autorizacao=='_') echo 'selected="select"'; ?>>Todos</option>
	</select>
	<label>Status:</label>
...
<div class="busca2">	
	...
	<input style="width: 90px; float: left" type="text" name="busca_data_f_a" value="<?  if($busca_data_f_a <> '') echo invert($busca_data_f_a,'/','PHP').' '.substr($busca_data_f_a,11, 8); ?>" class="form_estilo" />		
        ...
        <label>Fechado Entre: </label> 	
	<input type="text" name="busca_data_i_f" value="<? if($busca_data_i_f <> '') echo invert($busca_data_i_f,'/','PHP'); ?>" style="width: 90px; float: left" class="form_estilo" /> 	
	<strong	style="width: 10px; font-weight: bold; padding-top: 5px; float: left">e</strong>	
	<input type="text" name="busca_data_f_f" value="<?  if($busca_data_f_f <> '') echo invert($busca_data_f_f,'/','PHP'); ?>"	style="width: 90px; float: left" class="form_estilo" /> 		
...
<table width="100%" cellpadding="4" cellspacing="1" class="result_tabela">	
	...
	if($busca_e_forma=='') 			$p_valor .=  '<td align="center" width="50" class="result_menu"><b>Forma</b></td>';	
	if($busca_e_origem=='') 		$p_valor .=  '<td align="center" width="50" class="result_menu"><b>Origem</b></td>';	
	$p_valor .=  '
	...
	<td align="center" width="50" class="result_menu"><b>Lucro</b></td>
        <td align="center" width="50" class="result_menu"><b>Saldo</b></td>';
	...
	foreach($buscapedido as $p){		
		if($p->id_pedido_item<>''){			
			...
			$financeiro_sedex_total      = (float)($financeiro_sedex_total)+(float)($p->sedex);
			$financeiro_rateio_total      = (float)($financeiro_rateio_total)+(float)($p->rateio);
			$financeiro_custas_total      = (float)($financeiro_custas_total)+(float)($p->custas);
			$p->lucro = (float)$p->valor-(float)$p->sedex-(float)$p->custas-(float)$p->rateio;
			$p->saldo = (float)$p->valor-(float)$p->total;
                        if($p->saldo<0) $p->saldo=0;
			$financeiro_saldo_total      = (float)($financeiro_saldo_total)+(float)($p->saldo);
			...
			$p_valor .=  '
			<td class="result_celula" align="right" nowrap>R$ ' . number_format($p->custas,2,".","") . '</td>
			<td class="result_celula" align="right" nowrap>R$ ' . number_format($p->rateio,2,".","") . '</td>
			<td class="result_celula" align="right" nowrap>R$ ' . number_format($p->sedex,2,".","") . '</td>
			...
			<td class="result_celula" align="right" nowrap>R$ ' . number_format($p->lucro,2,".","") . '</td>
			<td class="result_celula" align="right" nowrap>R$ ' . number_format($p->saldo,2,".","") . '</td>';
			...
			if($controle_id_empresa == 1){			
				$atendente = $usuarios[$p->id_usuario];
				if($busca_e_atendimento=='') 	$p_valor .=  '<td class="result_celula" nowrap>' . $atendente . '</td>';	
			} else {
				if($busca_e_atendimento=='') 	$p_valor .=  '<td class="result_celula" nowrap>' . $p->atendente . '</td>';	
			}
			$p_valor .=  '</tr>';		
	...
	$financeiro_lucro_total            = (float)$valor_total-(float)$financeiro_sedex_total-(float)$financeiro_custas_total-(float)$financeiro_rateio_total;
	$valor_total            = 'R$ '.number_format($valor_total,2,".",",");	
	$financeiro_valor_total            = 'R$ '.number_format($financeiro_valor_total,2,".",",");
	$financeiro_sedex_total            = 'R$ '.number_format($financeiro_sedex_total,2,".",",");
	$financeiro_custas_total            = 'R$ '.number_format($financeiro_custas_total,2,".",",");
	$financeiro_rateio_total            = 'R$ '.number_format($financeiro_rateio_total,2,".",",");
	$financeiro_lucro_total            = 'R$ '.number_format($financeiro_lucro_total,2,".",",");
        $financeiro_saldo_total            = 'R$ '.number_format($financeiro_saldo_total,2,".",",");
	...
	$p_valor .=  '<td class="result_celula" align="right" nowrap>'.$financeiro_custas_total.'</td><td class="result_celula" align="right" nowrap>'.$financeiro_rateio_total.'</td><td class="result_celula" align="right" nowrap>'.$financeiro_sedex_total.'</td>
	<td class="result_celula" align="right" nowrap>'.$valor_total.'</td>	<td class="result_celula" align="right" nowrap>'.$financeiro_valor_total.'</td><td class="result_celula" align="right" nowrap>'.$financeiro_lucro_total.'</td><td class="result_celula" align="right" nowrap>'.$financeiro_saldo_total.'</td>';	
	if($busca_e_departamento=='') 	$p_valor .=  '<td class="result_celula" nowrap></td>';
