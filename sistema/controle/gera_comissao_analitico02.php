...
if ($_POST['submit']) {
	...
	for($i = 0; $i < count($lista); $i++){
		...
		$estilo['Align'] = 'left';
		$worksheet->setMerge($linha, 0, 0, 3);
		$worksheet->write($linha, 0, $lista[$i], $workbook->addFormat($estilo));
		$worksheet->write($linha, 1, '', $workbook->addFormat($estilo));
		$worksheet->write($linha, 2, '', $workbook->addFormat($estilo));
		
		if($i < 3){ $estilo['NumFormat'] = '_*0'; }
		else { $estilo['NumFormat'] = '_*R$ #,##0.00'; }
		$estilo['Align'] = 'center';
		$worksheet->write($linha, 3, $totais[$i], $workbook->addFormat($estilo));
