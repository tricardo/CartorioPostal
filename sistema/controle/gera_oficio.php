...
$cartorio_atual = $cartorio[$i];
	...
	foreach ($p_id_pedido_item as $chave => $id_pedido_item) {
		...
		$bloco = '';
        $res = $pedidoDAO->selectPorIdOficio($id_pedido_item,$controle_id_empresa);
...
//	    if($certidao_comarca_forum				            <>''){ $bloco .= " \par Comarca Fórum: ".$certidao_comarca_forum; $linha_bloco++; }
    	...
    	$res = $pedidoDAO->buscaPedidoPorId($id_pedido);
    	$nome						= $res->nome;
    	...
    	$res = $departamentoDAO->selectPorId($id_servico_departamento);
    	$departamento				= $res->departamento;
    	$res = $servicoDAO->selectPorId($id_servico);
    	$certidao_servico			= $res->descricao;

		$res = $cartorioDAO->selectPorId($cartorio_atual);
    	$cartorio_endereco		= '';
