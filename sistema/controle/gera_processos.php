...
foreach ($p_id_pedido_item as $chave => $id_pedido_item) {
	...
	$bloco = '';
	$sql = $objQuery->SQLQuery("SELECT pi.id_pedido, pi.certidao_cidade, pi.certidao_estado, pi.id_servico, pi.ordem, pi.certidao_nome, pi.certidao_cnpj, pi.certidao_cpf, s.descricao as servico, uu.nome as responsavel, uu.email as responsavel_email, 
	...
	e.id_empresa = uu.id_empresa and
	pi.id_pedido_item='" . $id_pedido_item . "' 
	and uu.id_usuario=pi.id_usuario and (uu.id_empresa='".$controle_id_empresa."' or pi.id_empresa_resp='".$controle_id_empresa."')");
	$res = mysql_fetch_array($sql);
...
//		if($controle_id_empresa=='1') 
		$responsavel_empresa    = $res['fantasia'];
		...
		if($anexar=='on' and $id_servico=='16'){
			...
			if($num_a==''){
				...
				$arq_anexo = $file_path.'decla_busca_'.$id_pedido_item.'_'.time().'.pdf';
				$pdf->Output($arq_anexo,'F'); //imprime a saida
				$sql = "insert into vsites_pedido_anexo (anexo,anexo_nome,id_pedido_item,id_usuario) values('$arq_anexo','Declaração de Busca','".$id_pedido_item."','".$controle_id_usuario."')";
