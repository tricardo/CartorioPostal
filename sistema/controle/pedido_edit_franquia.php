...
#verifica permissão de alterar o pedido e carrega variavel com data de encerramento
    $res = $pedidoverificaDAO->verificaPermissaoEditPedido($id_pedido, $ordem, 0);
...
#conta ordens, proximo, anterior, valor, fechadas e canceladas
    ...
    <form enctype="multipart/form-data" action="" method="get" name="pedido_outra_ordem" style="padding: 0; margin: 0">
        <h1 class="tit">
            ...
            &nbsp;&nbsp;&nbsp; <a href="pedido_franquias.php"> Buscar Pedidos</a> &nbsp;&nbsp;
            <?
...
<div id="meio">
    ...
    if(($submit_solicitante or $submit_servico or $submit_cartorio or $submit_deleta or $submit_atualiza_cart or $submit_anexo or $submit_anexo_deleta or $submit_financeiro_edit or $submit_financeiro_edit_r or $submit_financeiro_edit_r_d)){
        echo 'Somente o administrador pode efetuar essa correção';
        exit;
    }
    if ($submit_servico) {
        $aba = 'aba0';
    ...
    if ($submit_status) {//check for errors
		...
		$pagina_acessada = $_SERVER['SCRIPT_NAME'];
...
#alterado
    ...
    $p = $pedidoDAO->selectPedidoEditPorIdNovo($id_pedido, $ordem, '0', $p_campos);
    if ($p->id_pedido == '') {
        $dono = $pedidoDAO->buscaEmpResponsavel($id_pedido, $ordem);
        echo 'Você não tem permissão de alterar essa ordem, o dono é: <br/>'.$dono->fantasia;
        exit;
    }
    $rodape_pedido = '';
    if ($p->id_empresa != '0') {
