...
if($handle){
	while( ! feof($handle)){
		...
		if($qtdd_registros<4 or $qtdd_registros>5){
			if($qtdd_registros!=1)	$mail_mensagem .= '<li><b>LINHA '.$linha_cont.':</b> Quantidade de campos inválidos ('.$p_id_pedido[$cont].')</li>';
			continue;
		} else {
			if($linha_cont>1){
				...
				if(!is_numeric($p->id_pedido) or !is_numeric($p->ordem)){
					$mail_mensagem .= '<li><b>LINHA '.$linha_cont.':</b><br> Número do pedido inválido ('.$p_id_pedido[$cont].')</li>';
					continue;
				}
				...
				if(count($res_oficio)<>0){
					...
					if($p_verifica['error']=='' and $res_oficio->id_status==4){
						...
						$mail_mensagem_s .= '<li><b>Pedido '.$p_id_pedido[$cont].':</b><br> Atividade atualizada com sucesso</li>';
						$cont_liberado++;
					} else {
						$mail_mensagem .= '<li><b>Pedido '.$p_id_pedido[$cont].':</b><br> '.$p_verifica['error'].'</li>';
						$cont_erro++;
					}
				}
			}
