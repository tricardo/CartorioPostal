...
if($busca_id_pedido and $busca_ordem){
   $sql = $objQuery->SQLQuery("select pi.id_pedido, pi.ordem from vsites_pedido_item as pi where id_pedido = '".$busca_id_pedido."' and ordem = '".$busca_ordem."' limit 1");
   $res = mysql_fetch_array($sql);
   $ordem      	= $res['ordem'];
   $id_pedido 	= $res['id_pedido'];
   if($id_pedido<>''){
      echo '<meta HTTP-EQUIV="refresh" CONTENT="1; URL=pedido_edit.php?id='.$id_pedido.'&ordem='.$ordem.'">';
      exit;
   }
}
    ...
    <form name="buscador" action="" method="get" ENCTYPE="multipart/form-data">
        ...
        <div style="float:left; width:320px; text-align:right">
            ...
            <select name="busca_id_servico" style="width:200px; float:left" class="form_estilo">
                ...
                <?
                    $sql = $objQuery->SQLQuery("SELECT descricao, id_servico from vsites_servico as s order by descricao");
                    while($res = mysql_fetch_array($sql)){
                        echo '<option value="'.$res['id_servico'].'"';
						if($busca_id_servico==$res['id_servico']) echo ' selected="selected" ';
            ...
            <select name="busca_origem" style="width:200px; float:left" class="form_estilo">
                <option value="">Todos</option>
				<?
                    $sql = $objQuery->SQLQuery("SELECT distinct origem from vsites_origem as o where status!='Cancelado' order by origem");
                    while($res = mysql_fetch_array($sql)){
                        echo '<option value="'.$res['origem'].'"';
						if($busca_origem==$res['origem']) echo ' selected="selected" ';
            ...
            <select name="busca_id_status" style="width:200px; float:left" class="form_estilo">
				...
				<?
                    $sql = $objQuery->SQLQuery("SELECT distinct status, id_status from vsites_status as s order by status");
                    while($res = mysql_fetch_array($sql)){
                        echo '<option value="'.$res['id_status'].'"';
						if($busca_id_status==$res['id_status']) echo ' selected=;"selected" ';
            ...
            <select name="busca_id_atividade" style="width:200px; float:left" class="form_estilo">
                ...
                <?
                    $sql = $objQuery->SQLQuery("SELECT * from vsites_atividades as a where status!='Cancelado' order by atividade");
                    while($res = mysql_fetch_array($sql)){
                        echo '<option value="'.$res['id_atividade'].'"';
						if($busca_id_atividade==$res['id_atividade']) echo ' selected="selected" ';
						echo ' >'.$res['atividade'].'</option>';
                    }
                ?>
            </select> <br />
         ...
         <div style="float:left; width:290px; text-align:right">
            ...
            <select name="busca_id_departamento" style="width:200px" class="form_estilo">
                ...
                <?
                    $sql = $objQuery->SQLQuery("SELECT * from vsites_servico_departamento as sd where ordem='Sim' order by departamento");
                    while($res = mysql_fetch_array($sql)){
            ...
            <select name="busca_id_usuario" style="width:200px" class="form_estilo">
                ...
                <?
                    $sql = $objQuery->SQLQuery("SELECT * from vsites_user_usuario as uu where id_empresa='".$controle_id_empresa."' and status!='Cancelado' and 6 IN (departamento_p)  order by nome");
                    while($res = mysql_fetch_array($sql)){
            ...
            <select name="busca_id_usuario_op" style="width:200px" class="form_estilo">
                ...
                <?
                    $sql = $objQuery->SQLQuery("SELECT id_usuario, nome from vsites_user_usuario as uu where id_empresa='$controle_id_empresa' and status!='Cancelado' order by nome");
                    while($res = mysql_fetch_array($sql)){
                        echo '<option value="'.$res['id_usuario'].'"';
						if($busca_id_usuario_op==$res['id_usuario']) echo ' selected="selected" ';
						echo ' >'.$res['nome'].'</option>';
                    }
                ?>        
            </select>
