...
#	$empresas = $empresaDAO->listarTodasCronRoy((isset($_GET['id_user_empresa']) ? $_GET['id_user_empresa'] : 0));
	$empresas = $empresaDAO->listarTodasCronRoy();

	echo '<pre>';
	$cont=0;
	foreach($empresas as $emp){
		#if($emp->id_empresa > 320 AND $emp->id_empresa != 330){//tirar
		#if($emp->id_empresa > 375){//tirar
					#
					#
					#
					#print_r($empresas);
					...
					echo 'passou aqui - '; 
					echo "\n".$emp->id_empresa." fazendo rel de ".$fantasia."\t\t".$imposto.'<br>';
					...
					$b->vencimento=date('Y-m').'-08';//alterar
					$b->especie='12';
					$b->aceite='N';
					$b->mensagem1='';
