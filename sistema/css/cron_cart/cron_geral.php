...
while($retorna>0){
	...
	$ano_mes = date('Y-m',strtotime("+ 1 day -".$retorna." month"));
	$ano_mes_exp = explode('-',$ano_mes);	
	$mes_referencia = date('m/Y',strtotime("+ 1 day -".$retorna." month"));
...
foreach($empresas as $emp){
	...
	#total de pedidos
	$queryrel 	= $objQuery->SQLQuery("select * FROM `vsites_relatorios` WHERE `descricao` = 'relatório geral' and data_relatorio='".$ano."-".$mes."-01' and id_empresa='".$id_empresa."'");
	$num_totalrel = mysql_num_rows($queryrel);
	if($num_totalrel>0) {
		echo '<br>Tem Empresa '.$emp->id_empresa.'= '.$mes.'-'.$ano;
		continue;
	}
	echo '<br>Não tem Empresa '.$emp->id_empresa.'= '.$mes.'-'.$ano;
