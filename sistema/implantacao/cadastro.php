...
if (isset($c->submit)) {
    ...
    if ($c->tipo == 'cpf') {
        ...
        if ($valida == 'false' and $c->franquia_tipo != 4) {
            $errors['cpf'] = 1;
            $error .= "<li><b>CNPJ Inválido, digite corretamente.</b></li>";
        }
    }
    $c->notificacao = str_replace("\n", '<br />', $c->notificacao);
    $emp = $c;
    if (count($errors) == 0) {
        if ($c->id_empresa > 0) {
            $empresaDAO->atualizar($emp);

            if($c->id_royalties_fixo_franquiado > 0){
                $royalties->altera_royalties_fixo($c);
            }else{
                $royalties->insert_royalties_fixo($c);
            }
...
<select name="status" class="form_estilo" <?= ($permissao_admin == 'TRUE' or $permissao_Franchising == 'TRUE') ? '' : 'disabled="disabled"' ?>>
    ...
    <option value="Jurídico" <?= ($emp->status == 'Jurídico') ? 'selected="selected"' : '' ?>>Jurídico</option>
    <option value="Renovação" <?= ($emp->status == 'Renovação') ? 'selected="selected"' : '' ?>>Renovação</option>
</select>
...
<select onchange="franquia_master(this.value)" name="franquia_tipo"
    ...
    <option value="4" <?= ($emp->franquia_tipo == 4) ? 'selected="selected"' : '' ?>>Internacional</option>
</select><br/>
...
<? if ($emp->franquia_tipo != 1) { ?>
    ...
    <select style="width:562px" name="id_recursivo" id="id_recursivo"
        ...
        foreach ($dt as $f) { ?>
            <option
                value="<?= $f->id_empresa ?>" <?= ($f->id_empresa == $emp->id_recursivo) ? 'selected="selected"' : '' ?>><?= $f->fantasia ?></option>
        <? } ?>
...
<input type="text" name="nome" value="<?= $emp->nome ?>" style="margin-left:1px;width:542px"
       class="form_estilo <?= (isset($errors['nome'])) ? 'form_estilo_erro' : ''; ?>"/><font color="#FF0000">*</font>
<br/>
...
<div id="cpf" style="float: left"><input type="text" name="cpf" value="<?= $emp->cpf ?>" style="width: 150px"
                                         onKeyUp="if(tipo.value=='cpf') masc_numeros(this,'###.###.###-##'); else if(franquia_tipo.value!=4) masc_numeros(this,'##.###.###/####-##'); else  masc_numeros(this,'################');"
...
<input type="text" name="tel" value="<?= $emp->tel ?>"
       style="width: 130px" onKeyUp="if(franquia_tipo.value!=4)masc_numeros(this,'(##) ####-####'); else '';"
...
<input type="text" name="cel" value="<?= $emp->cel ?>"
       ...
       onKeyUp="if(franquia_tipo.value!=4)masc_numeros(this,'(##) ####-####'); else '';" class="form_estilo"/><br/>
...
<label>Skype:</label>
<input type="text" name="skype" value="<?= $emp->skype ?>" style="margin-left:1px;width:200px" class="form_estilo"/>
<br/><br/>
...
<input type="text" name="cep" style="width: 150px" value="<?= $emp->cep ?>"
       ...
       onKeyUp="if(franquia_tipo.value!=4) masc_numeros(this,'#####-###');"/> <font color="#FF0000">*</font>
...
<label>Agência:</label>
<input type="text" name="agencia" style="margin-left:1px;" value="<?= $emp->agencia ?>" class="form_estilo"
       maxlength="15"/>

<label>Conta:</label>
<input type="text" name="conta" style="width:200px;margin-left:1px" value="<?= $emp->conta ?>" class="form_estilo"
       maxlength="15"/><br/>

<label>Favorecido:</label>
<input type="text" name="favorecido" style="width:543px" value="<?= $emp->favorecido ?>" class="form_estilo"
       maxlength="255"/><br/><br/>
...
<div style="width:17px; text-align:center; float:left; padding:1px;margin-left:-1px;">
    <input type="checkbox" class="form_estilo" id="adendo" name="adendo"
           value="1" <?= ($emp->adendo) ? 'checked="checked"' : '' ?>/>
</div>
<input type="text" name="adendo_data"
       value="<?= ($emp->adendo_data != '') ? invert($emp->adendo_data, '/', 'PHP') : ''; ?>"
       style="width: 183px" onkeyup="masc_numeros(this,'##/##/####');"
       class="form_estilo  <?= (isset($errors['adendo_data'])) ? 'form_estilo_erro' : ''; ?>"
       id="adendo_data"/>
<span id="obrig"><font color="#FF0000">*</font></span><br/>

<label>Início de Contrato:</label>
<input type="text" name="inauguracao_data"
       value="<?= ($emp->inauguracao_data != '') ? invert($emp->inauguracao_data, '/', 'PHP') : ''; ?>"
       onkeyup="masc_numeros(this,'##/##/####');"
       class="form_estilo  <?= (isset($errors['inauguracao_data'])) ? 'form_estilo_erro' : ''; ?>"
       id="inauguracao_data"/>

<label>Final de Contrato:</label>
<input type="text" name="validade_contrato"
       value="<?= ($emp->validade_contrato != '') ? invert($emp->validade_contrato, '/', 'PHP') : ''; ?>"
...
<input type="text" name="precontrato"
       value="<?= ($emp->precontrato != '') ? invert($emp->precontrato, '/', 'PHP') : ''; ?>"
       onkeyup="masc_numeros(this,'##/##/####');"
       class="form_estilo  <?= (isset($errors['precontrato'])) ? 'form_estilo_erro' : ''; ?>" id="inauguracao_data"/>
...
<input type="text" name="exclusividade" value="<?= $emp->exclusividade; ?>" style="width: 150px"
       onkeyup="masc_numeros(this,'##');"
       class="form_estilo  <?= (isset($errors['exclusividade'])) ? 'form_estilo_erro' : ''; ?>"/>(meses)<br/>

<label>Notificação (Data,Motivo):</label>
<textarea name="notificacao" style="width:539px; height:100px"
...
<input type="text" name="imposto" value="<?= ($emp->imposto == '') ? 0 : $emp->imposto ?>" id="imposto"
       style="width: 50px"
...
<? if ($c->id_empresa > 0) {
    ...
    <input type="text" name="data_royalties" id="data_royalties"
           ...
           class="form_estilo" disabled="disabled"/>

    <?
    $cUsuarioDAO = new UsuarioDAO();
    $cUsuario = $cUsuarioDAO->selectPorId($c->id_usuario);
    ?>

    <label style="font-weight:normal">Alterado Por</label>
    <input type="text" name="alt_royal" id="alt_royal" value="<?= ($cRoyalties->id_royaltie_fixo_franquiado > 0) ? $cRoyalties->nome : $cUsuario->nome; ?>"
           onkeyup="moeda(event.keyCode,this.value,this.id);"  style="width:142px; margin:0"
           class="form_estilo" disabled="disabled"/>
    <input type="hidden" name="id_royalties_fixo_franquiado" id="id_royalties_fixo_franquiado" value="<?= $cRoyalties->id_royaltie_fixo_franquiado ?>">
    <br/>
...
<div style="text-align:center;width:100%">
    <? if ($c->id_empresa > 0) { ?>
        <input style="float:none;margin:0" type="button" name="submit" value="Atualizar" class="button_busca"
               onclick="franquia_editar(1,<?= $c->id_empresa ?>)"/>
        <input style="float:none;margin:0" type="button" name="cancelar" value="Cancelar"
               onclick="javascript:history.go(-1);" class="button_busca"/>
        <input style="float:none;margin:0" type="button" name="mensagens" value="Mensagens"
               onclick="location.href='empresa_msg.php?id_empresa=<?php echo $emp->id_empresa ?>'"
               class="button_busca"/>
    <? } else { ?>
        <input style="float:none;margin:0" type="button" name="submit" value="Cadastrar" class="button_busca"
               onclick="franquia_editar(1,<?= $c->id_empresa ?>)"/>
        <input style="float:none;margin:0" type="button" name="cancelar" value="Cancelar"
               onclick="javascript:history.go(-1);" class="button_busca"/>
    <? } ?>
