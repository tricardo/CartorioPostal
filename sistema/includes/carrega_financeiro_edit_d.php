...
<table width="650" class="tabela">
	...
	<tr>
		...
		<td colspan="3"><select name="financeiro_banco" style="width: 450px"
			...
			<option value=""></option>
			<?
			$sql = $objQuery->SQLQuery("SELECT * from vsites_banco order by banco");
			while($res = mysql_fetch_array($sql)){
	...
	<tr>
		...
		<center><input type="hidden" name="financeiro_old_autorizacao"
			...
			type="hidden" name="id_pedido_item" value="<?= $id_pedido_item ?>" />
		<input type="submit" class="button_busca"
			name="submit_financeiro_edit" value="Atualizar" /></center>
