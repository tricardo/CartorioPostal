...
if($handle){
	while( ! feof($handle)){
		...
		if ($qtdd_registros<>20 and $qtdd_registros!=0 and $qtdd_registros!=1) $erro.='Quantidades de campos inválido na linha '.$linha_cont.'-'.$qtdd_registros.'<br>';
		if($linha_cont==1) {
			$linha_cont++;
			CONTINUE;
		}
		...
		$R_VALOR = trim(str_replace('  ','',$buffer[18]));
		$R_CORRECAO = trim(str_replace('  ','',$buffer[19]));
