...
if($conveniado_tabela){
	$sql = $objQuery->SQLQuery("SELECT c.*, cc.id_cliente, cc.id_conveniado, cc.contato, c.id_cliente, uu.id_empresa, uu.id_usuario FROM vsites_user_conveniado as cc, vsites_user_cliente as c, vsites_user_usuario as uu WHERE cc.email = '".$conveniado_login."' and cc.senha='".$conveniado_senha."' and cc.status='Ativo' and c.id_cliente= cc.id_cliente and c.status='Ativo' and c.id_usuario=uu.id_usuario");
	$rowconv = mysql_fetch_array($sql);
    $conveniado_nome = $rowconv['contato'];
    ...
    $conveniado->id_usuario = $rowconv['id_usuario'];
    $conveniado->nome = $rowconv['nome'];
