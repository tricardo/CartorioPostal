...
class EmpresaDAO extends Database {
    ...
    public function listarTodasRoy() {
        ...
        if ($verifica == false) {
            $this->sql = "SELECT id_empresa, CONCAT(fantasia,' - ',UPPER(status)) AS fantasia from vsites_user_empresa as uu where id_empresa!=1 order by fantasia";
            $this->values = array();
            $ret = $this->fetch();
            $campos = "id_empresa;fantasia";
            $geracsv = $cachediaCLASS->ConvertArrayToCsv($filename, $ret, $campos);
