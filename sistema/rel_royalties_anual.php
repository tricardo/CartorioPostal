	...
	//for($i = 4; $i < count($abas); $i++){
		...
		if($i <= 3){
				...
				for($j = 0; $j < 14; $j++){
					if($j == 0){
						...
						if($soma_xy[$j]){
							...
							$total_xy[$j] = $total_xy[$j] + (float)($soma_xy[$j]);
							
							$linha_x = $linha_x + (float)($soma_xy[$j]);
							$worksheet->write($total, $j, (float)($soma_xy[$j]), $estilo);
						} else {
							if(!$total_xy[$j]) $total_xy[$j] = 0;
							$total_xy[$j] = $total_xy[$j] + 0;
			...
			for($j = 1; $j <= count($ref)-2; $j++){
				$total_anual = $total_anual + (float)($total_xy[$j]);
			...
			for($k = 0; $k < count($dt); $k++){	
				$total++;
				$mes   = 0;
				$ryrc  = explode(';', $dt[$k]['valor_royalties']);
				$fppr  = explode(';', $dt[$k]['fpp']);	
				$ryrd  = explode(';', $dt[$k]['roy_rec']);	
				$fprc  = explode(';', $dt[$k]['fpp_rec']);
				...
				for($j = 0; $j <= 48; $j++){
					...
					if($j == 0){
						...
						$teste = ($k == 0) ? $estilo1 = $style22 : $estilo1 = $style20;
						$teste = ($ryrc[$mes] == '') ? $ryrc[$mes] = (float)0 : $ryrc[$mes] = (float)$ryrc[$mes];;
						$teste = ($fppr[$mes] == '') ? $fppr[$mes] = (float)0 : $fppr[$mes] = (float)$fppr[$mes];
						$teste = ($ryrd[$mes] == '') ? $ryrd[$mes] = (float)0 : $ryrd[$mes] = (float)$ryrd[$mes];
						$teste = ($fprc[$mes] == '') ? $fprc[$mes] = (float)0 : $fprc[$mes] = (float)$fprc[$mes];
						switch(fmod($j, 4)){
							case 1: 
								$total_roy_r = ($ryrc[$mes] - $ryrd[$mes]<0)?$total_roy_r=0:$total_roy_r=$ryrc[$mes] - $ryrd[$mes];
								$worksheet->write($total, $j, ($total_roy_r), $estilo1);
								$ryrc_x = $ryrc_x + ($total_roy_r );
								$ryrc_y[$j] = $ryrc_y[$j] + ($total_roy_r );
							break;
							case 2: 
								$total_fpp_r = ($fppr[$mes] - $fprc[$mes]<0)?$total_fpp_r=0:$total_fpp_r=$fppr[$mes] - $fprc[$mes];
								$worksheet->write($total, $j, $total_fpp_r, $estilo1);
								$fppr_x = $fppr_x + ($total_fpp_r );
								$fppr_y[$j] = $fppr_y[$j] + ($total_fpp_r );
							break;
							case 3: 
								$worksheet->write($total, $j, $ryrd[$mes], $estilo1); 
								$ryrd_x = $ryrd_x + $ryrd[$mes];
								$ryrd_y[$j] = $ryrd_y[$j] + $ryrd[$mes];
							break;
							case 0: 
								if($k == 0){
								...
								$fprc_x = $fprc_x + $fprc[$mes];
								$fprc_y[$j] = $fprc_y[$j] + $fprc[$mes];
								$mes++;	
							break;
						}
					}	
				...
				$ryrc_y[49] = $ryrc_y[49] + $ryrc_x;
				$fppr_y[50] = $fppr_y[50] + $fppr_x;
				$ryrd_y[51] = $ryrd_y[51] + $ryrd_x;
				$fprc_y[52] = $fprc_y[52] + $fprc_x;
				$worksheet->write($total, 53, '', $style15);
			}
...
<div id="topo">
	<h1 class="tit">
	<img src="../images/tit/tit_cartorio.png" alt="Título" />Relatório Anual Consolidado</h1>
	<hr class="tit" /><br />
</div>
<div id="meio">
