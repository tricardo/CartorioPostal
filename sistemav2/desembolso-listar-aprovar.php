 ...
 if(in_array('2',$departamento_p)==1){
    if(isset($_SESSION) AND isset($_SESSION['desembolso']) AND is_array($_SESSION['desembolso']) AND
        ...
        for($i = 0; $i < count($_SESSION['desembolso']); $i++){
            ...
            $id_pedido      = $items[2];
            $ordem          = $items[3];
            $dt = $financeiroDAO->aprovaDesembolso($id_pedido_item,$id_financeiro,$controle_id_usuario,$controle_id_empresa);
            if($dt <> 0){
                if($ok == ''){ $ok = 'Desembolso Aprovado para:<br><br>'; }
                $pedido[] = '#'.$id_pedido.'/'.$ordem;
            } else {
                if($erro == ''){ $erro = 'Desembolso Não Aprovado para:<br><br>'; }
