...
foreach($financeiroDAO->arquivo($controle_id_empresa, $onde) AS $res){
    ...
    $cpf = isset($res['financeiro_cpf']) ? substr(str_replace('/','',str_replace('-','',str_replace('.','',$res['financeiro_cpf']))),0,15) : '';
    $carac = strlen($cpf);
...
if(fopen($arquivoDiretorio,"w+")) {
   ...
   if(!fwrite($handle, $arquivoConteudo)) {
    ...
    $financeiroDAO->arquivo($controle_id_empresa, $onde, 2);
