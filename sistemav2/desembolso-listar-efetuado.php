    ...
    for($i = 0; $i < count($_SESSION['desembolso']); $i++){
        ...
        if($valida=='TRUE'){
            ...
            if(!isset($p_verifica['error']) OR $p_verifica['error']==''){
                $financeiroDAO->retornoDeposito($id_pedido_item,$id_financeiro,$controle_id_usuario);
                $pedido[] = '#'.$id_pedido.'/'.$ordem.' - retorno do depósito efetuado;<br>';
                if($ok == ''){ $ok = 'Depósito do Desembolso Efetuado para:<br><br>'; }
            } else {
                if($erro == ''){ $erro = 'Problema para Efetuar Depósito do Desembolso para:<br><br>'; }
