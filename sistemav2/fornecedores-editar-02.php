...
if(isset($_GET['excluir_anexo']) AND is_numeric($_GET['excluir_anexo'])){
    $exc = new stdClass();
    $exc->id_fornecedor_anexo = $_GET['excluir_anexo'];
    $fornecedorDAO->excluirAnexo($exc);
    $exc_anexo = 1;
    $msgbox .= MsgBox(3);
}
...
if($_POST AND isset($_POST['f_anexo'])){
	...
	$config["altura"]  = 1024;
        ...
        if($errors == 0){
            ...
            $fornecedorDAO->inserirAnexo($a);
            $msgbox .= MsgBox(2);
        }
    }
   
}
$fld = date('Y-m-d') <= $data_init_cp ? 'anexos_fornecedor/' : '../sistema/anexos_fornecedor/';
?>
<form enctype="multipart/form-data" method="post" id="form2" action="<?=$link?>&opcoes_form=2">
    ...
    <?php 
    $anexos = $fornecedorDAO->buscaAnexosForn($id_fornecedor, $controle_id_empresa);
    if(count($anexos) > 0){ ?>
    <h3>anexos</h3>
    <dl class="box">
         <table class="table1">
                ...
                foreach ($anexos as $anexo) { 
                    ...
                    <tr <?=TRColor($color)?>>
                        ...
                        <td class="size100">
                            <a href="<?=$link?>&opcoes_form=2&excluir_anexo=<?= $anexo->id_fornecedor_anexo ?>">
...
<?php if(isset($_POST['f_anexo']) OR isset($exc_anexo)){ ?>
    ...
    <script>
        BoxMsg(<?=($_POST OR isset($exc_anexo)) ? 1 : 0?>,<?=$errors?>,'<?=$campos?>','<?=$msgbox?>');
    </script>
<?php
}
$errors=0;
