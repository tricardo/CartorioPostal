...
class ContaDAO extends Database{
	...
	public function listar($id_empresa, $busca="",$pagina=""){
		$this->sql = "SELECT count(0) as total FROM vsites_conta co WHERE id_empresa=? ";
                $this->sql .= defined('MOSTRAR_CONTAS') ? '' : " AND co.status='Ativo'";
                $this->values = array($id_empresa);
                if(strlen($busca) > 0){
                    $this->sql .= "AND (";
                    $this->sql .= "co.sigla LIKE ? OR ";
                    $this->sql .= "co.agencia LIKE ? OR ";
                    $this->sql .= "co.conta LIKE ? OR ";
                    $this->sql .= "co.status LIKE ?";
                    $this->sql .= ") ";
                    $this->values[] = $busca."%";
                    $this->values[] = $busca."%";
                    $this->values[] = $busca."%";
		...
		$this->sql = "SELECT c.*, b.banco FROM vsites_conta c
                ...
                if(strlen($busca) > 0){
                    $this->sql .= "AND (";
                    $this->sql .= "c.sigla LIKE ? OR ";
                    $this->sql .= "c.agencia LIKE ? OR ";
                    $this->sql .= "c.conta LIKE ? OR ";
                    $this->sql .= "c.status LIKE ?";
                    $this->sql .= ") ";
                    $this->values[] = $busca."%";
                    $this->values[] = $busca."%";
                    $this->values[] = $busca."%";
	...
	public function selectBoletosBrad($id_empresa){
		$this->sql = "SELECT cf.*, date_format(cf.vencimento,'%d%m%y') vencimento, date_format(cf.emissao,'%d%m%y') emissao,  date_format(cf.data_desc,'%d%m%y') data_desc FROM vsites_conta_fatura cf 
					WHERE status=0 and id_empresa=? ";
		$this->values = array($id_empresa);
		return $this->fetch();
	}

	public function selectBoletosBradOco($id_empresa){
		$this->sql = "SELECT cf.*, cfo.id_conta_fatura_oco, (CASE WHEN cfo.vencimento!='0000-00-00' THEN date_format(cfo.vencimento,'%d%m%y') ELSE date_format(cf.vencimento,'%d%m%y') END) as vencimento, date_format(cf.emissao,'%d%m%y') emissao,  date_format(cf.data_desc,'%d%m%y') data_desc, 
				...
				FROM vsites_conta_fatura_oco cfo, vsites_conta_fatura as cf 
					WHERE cfo.status=0 and cfo.id_empresa=? and cfo.id_conta_fatura=cf.id_conta_fatura";
		$this->values = array($id_empresa);
		return $this->fetch();
	}

	public function selectBoletosBradPorId($id_conta_fatura,$id_empresa){
		$this->sql = "SELECT cf.*, ce.sigla, ci.conta_instru, c.favorecido, c.agencia, c.conta, c.carteira FROM vsites_conta_fatura cf LEFT JOIN vsites_conta_instru ci ON cf.instrucao1=ci.id_conta_instru LEFT JOIN vsites_conta_especie ce on ce.id_conta_especie=cf.especie, vsites_conta as c
	...
	public function selectBoletosBradFat($id_fatura,$id_empresa){
		...
		$this->sql = "SELECT cf.id_conta_fatura FROM vsites_conta_fatura cf 
                    LEFT JOIN vsites_conta_instru ci ON cf.instrucao1=ci.id_conta_instru 
                    LEFT JOIN vsites_conta_especie ce on ce.id_conta_especie=cf.especie, vsites_conta as c
	...
	public function execSession($sessao){
        ...
        public function remessas($acao = 1){
            global $controle_id_empresa;
            switch($acao){
                case 1:
                    $this->sql = "SELECT * from vsites_arquivo_brad_rem where id_empresa=? order by data desc limit 25";
                    $this->values = array($controle_id_empresa);
                    return $this->fetch();
                    break;
                
                case 2:
                    $this->sql = "SELECT * from vsites_arquivo_brad_ret where id_empresa=? order by data desc limit 25";
                    $this->values = array($controle_id_empresa);
                    return $this->fetch();
                    break;
            }
