...
class CorreioDAO extends Database {
    ...
    public function listar($id_empresa, $busca, $pagina="") {
        ...
        $this->link = strlen($busca) > 0 ? 'busca=' . $busca : '';
    ...
    public function atualizar($con,$id) {
        ...
        $this->sql = "UPDATE vsites_agcorreios SET nome=?, status=?, data_cartaz=?, id_empresa=?, "
                . "endereco=?, bairro=?, cidade=?, estado=?, cep=?, tel=?, fax=? WHERE id_agcorreios = ? ";
        $this->values = array($con->nome, $con->status, $con->data_cartaz, $con->id_empresa, 
            $con->endereco, $con->bairro, $con->cidade, $con->estado, $con->cep, $con->tel, $con->fax, $id);
        return $this->exec();
    }
