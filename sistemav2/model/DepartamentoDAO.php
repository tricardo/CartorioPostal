...
class DepartamentoDAO extends Database{
	...
	public function listar($ids=null){
		...
		$this->sql.=' ORDER BY departamento';
                if($ids==null){
                    ...
                    $filename = 'DepartamentoDAO-listar.csv';
                    if(!$cachediaCLASS->VerificaCache($filename)){
                        $ret = $this->fetch();
                        $campos = "id_departamento;departamento;ticket;franqueadora";
	...
	public function listarDptoUser($id_empresa){
		...
		$cachediaCLASS = new CacheDiaCLASS();
                $filename = 'DepartamentoDAO-listarDptoUser-'.$id_empresa.'.csv';
                if(!$cachediaCLASS->VerificaCache($filename)){
                    $ret = $this->fetch();
                    $campos = "id_departamento;departamento;ticket;franqueadora";
                    $cachediaCLASS->ConvertArrayToCsv($filename,$ret,$campos);
	...
	public function carregarCombo(){
            ...
            $this->values = array();
            $cachediaCLASS = new CacheDiaCLASS();
            $filename = 'DepartamentoDAO-carregarCombo-'.$id_empresa.'.csv';
            if(!$cachediaCLASS->VerificaCache($filename)){
                $ret = $this->fetch();
                $campos = "id_servico_departamento;departamento;status;id_departamento_resp;ordem";
                $cachediaCLASS->ConvertArrayToCsv($filename,$ret,$campos);
            } else {
                $ret = $cachediaCLASS->ConvertCsvToArray($filename, array("to_object" => true));
            }
