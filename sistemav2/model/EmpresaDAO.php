...
class EmpresaDAO extends Database {
    ...
    public function planejamento_economico($id_empresa = 0){
        $this->sql = "select *, date_format( str_to_date(mes, '%Y-%m'),'%m/%Y') as mes 
            from vsites_rel_viabilidade as rv 
    ...
    public function listarTodas($ids=null) {    
        ...
        $filename = 'EmpresaDAO-listarTodas-'.$controle_id_empresa;
        $filename .=(count($vars) > 0) ? '-'.implode('',$vars) : '';
        $filename .= '.csv';
        if(!$cachediaCLASS->VerificaCache($filename)){
            $ret = $this->fetch();
            $campos = "id_empresa;fantasia;imposto;royalties;empresa;cpf;tipo;cep;endereco;complemento;"
                    . "numero;inicio;sem1;sem2;sem3;roy_min;roy_min2;inauguracao_data;validade_contrato;email";
    ...
    public function listarTodasCronRoy($uid = 0) {
        $this->sql = "SELECT ue.id_empresa, TRIM(REPLACE(fantasia,'Cartório Postal - ','')) as fantasia , 
				imposto, royalties,fpp,fpp_tipo, empresa, cpf, tipo, cep, endereco, complemento, numero, inicio, sem1, 
				sem2, sem3, inauguracao_data,validade_contrato,email, rf.mes_1, rf.mes_2, rf.mes_3, rf.mes_4, 
				rf.mes_5, rf.mes_6, rf.mes_7, rf.mes_8, rf.mes_9, rf.mes_10, rf.mes_11, rf.mes_12, rf.mes_13, rf.mes_14, 
				rf.mes_15, rf.mes_16, rf.mes_17, rf.mes_18
			FROM vsites_user_empresa as ue LEFT JOIN vsites_royaltie_fixo as rf ON ue.id_empresa=rf.id_empresa 
			WHERE ue.status = 'Ativo' ";
		$this->sql .= $uid > 0 ? ' AND ue.id_empresa = ?' : '';
        $this->values = $uid > 0 ? array($uid) : array();
        $this->sql .= " ORDER BY ue.id_empresa";
        return $this->fetch();
    }
    ...
    public function listarTodasFranquias() {
        $this->sql = "SELECT SQL_CACHE id_empresa, TRIM(REPLACE(fantasia,'Cartório Postal - ','')) as fantasia ,tel, fax, cep, endereco, complemento, numero, bairro, cidade, estado
						FROM vsites_user_empresa as ue ORDER BY fantasia";
        $this->values = array();
        return $this->fetch();
    }
    ...
    public function listarTodasRoy() {
        ...
        if ($verifica == false) {
            $this->sql = "SELECT id_empresa, CONCAT(fantasia,' - ',UPPER(status)) AS fantasia from vsites_user_empresa as uu where id_empresa!=1 and (status='Ativo' or status='Cancelado' or status='Inativo') order by fantasia";
            $this->values = array();
            $ret = $this->fetch();
            $campos = "id_empresa;fantasia";
            $geracsv = $cachediaCLASS->ConvertArrayToCsv($filename, $ret, $campos);
    ...
    public function inserir($empresa) {
        $this->values = array('fantasia' => $empresa->fantasia,
            ...
            'tel' => $empresa->tel, 'email' => $empresa->skype, 'email' => $empresa->skype,
            ...
            'adendo' => $empresa->adendo,'ip'=>$empresa->ip,
    ...
    public function atualizar($empresa) {
        ...
        $this->values = array(
            ...
            $empresa->email, $empresa->skype, $empresa->endereco,
            $empresa->bairro, $empresa->cidade,
            $empresa->estado, $empresa->cep,
            '0000-00-00', $empresa->cpf,
            $empresa->rg, $empresa->empresa,
            $empresa->data_hotsite, $empresa->complemento,
            $empresa->numero, $empresa->ramal,
            $empresa->status, ($empresa->franquia_tipo != 1 ? 'sim' : 'não'),
            ...
            $empresa->notificacao, $empresa->precontrato,(strlen($empresa->cpf) > 14 ? 'cnpj' : 'cpf'),
            $empresa->aditivo,$empresa->franquia_tipo,
			$empresa->id_recursivo,$empresa->ip);
