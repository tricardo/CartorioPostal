<?php class ExpansaoDAO extends Database {
	...
	public function verAcessoExec($exp_item){
        ...
        public function pipeline($c){
            $this->sql = "SELECT COUNT(0) as total, id_status FROM site_ficha_cadastro as fc WHERE 1=1";
            if(isset($c->ano) AND strlen($c->ano) > 0){
                $this->sql .= ' AND fc.data LIKE ?';
                $this->values = array(
                    ((isset($c->mes) AND strlen($c->mes) > 0) ? $c->ano.'-'.$c->mes.'%' : $c->ano.'%')
                );
            }
            $this->sql .= ' group by id_status';
            return $this->fetch();
        }
	...
	public function verAcesso($controle_acao, $controle_id_empresa, $controle_id_usuario, $controle_id_departamento_p, 
		...
		if($retorno->id_empresa == 1){
			switch($controle_acao){
				...
				case 3:
					...
					$retorno->id_users = array(1,56,828,2145);
					break;
				case 4:
					$retorno->id_depto = array(0);
					$retorno->id_users = $retorno->id_users_extra;
		...
		if(in_array($retorno->id_usuario,$retorno->id_users)){
			$retorno->acesso = 1;
                    ...
                    $depto = array();
                    if(isset($retorno->id_departamento_p) AND !is_array($retorno->id_departamento_p)){
                        $depto = (substr_count($retorno->id_departamento_p,",") > 0) ? 
                            explode(',', $retorno->id_departamento_p) : 
                            (strlen($retorno->id_departamento_p) > 0 ? array($retorno->id_departamento_p) : array());
                    } elseif(isset($retorno->id_departamento_p) AND is_array($retorno->id_departamento_p)){
                        $depto = $retorno->id_departamento_p;
                    }
	...
	public function carregar_consultor($exp_item, $c){
		$combosel = 0;
		if(isset($exp_item->consultor2) AND $exp_item->consultor2 == 1){
                    $combosel = 1;
		}
		...
		switch($exp_item->id_usuario){
			...
			default:
				...
				if($exp_item->acesso == 0){
                                    $SQL .= ' AND u.id_usuario = ' . (isset($c->c_id_usuario) ? $c->c_id_usuario : $exp_item->id_usuario) . ' ';
				}
		}
        ...
        $dt = $this->fetch();
                $cont = 0;
		if($combosel == 1){
			echo '<select id="consultor2" name="consultor2"class="chzn-select">'."\n";
			foreach($dt as $b => $res){ 
				echo (count($dt) > 1 && $cont == 0) ? '<option value="0">--</option>' . "\n" : ''; 
				$cont++;
				echo "\t\t\t\t\t".'<option value="'.$res->id_usuario.'"'.((((int)$c->consultor2) == $res->id_usuario) ? 
					' selected="selected"' : '').'>'.
					(($exp_item->id_usuario == 1) ? '['.$res->id_usuario.'] - ':'').utf8_encode($res->nome).'</option>' . "\n";
			}
			echo '</select>'."\n";
		} else {
			echo '<select id="consultor" name="consultor"class="chzn-select">'."\n";
			foreach($dt as $b => $res){ 
				echo (count($dt) > 1 && $cont == 0) ? '<option value="0">--</option>' . "\n" : ''; 
	...
	public function carregar_status($exp_item, $c){
        ...
        $dt = $this->fetch();
                $cont = 0;
		echo '<select name="id_status" id="id_status"class="chzn-select">'."\n";
		foreach($dt as $b => $res){ 
			echo (count($dt) > 1 && $cont == 0) ? '<option value="0">--</option>' . "\n" : ''; $cont++;
			echo "\t\t\t\t\t".'<option value="'.$res->id_status.'"'.((((int)$c->id_status) == $res->id_status) ? 
				' selected="selected"' : '').'>'.
				(($exp_item->id_usuario == 1) ? '['.$res->id_status.'] - ':'').utf8_encode($res->status).'</option>' . "\n";
		}
		echo '</select>'."\n";
	}
	...
	public function consulta($exp_item, $c){
		...
		if($exp_item->acesso == 0){
			...
			$CONDICAO .= ' AND c.id_usuario = ' . (isset($c->c_id_usuario) ? $c->c_id_usuario : $exp_item->id_usuario) . ' ';
		} else {
			if($exp_item->id_usuario != 1){
				$CONDICAO .= ' AND s.id_status != 20 ';
			}
	...
	public function excluir_duplicidade($id_usuario, $fichas){
...
#			);
	...
	public function email_finalizar_processo($c){
		...
		São Paulo, '.date('d').' de '.mes_ext(date('m')).' de '.date('Y').'.<br /> 
		...
		if(PRODUCAO == 0){
                    ...
                    $AddAddress = lista_emails('juridico');
	...
	public function email_finalizar($c){
		...
		if($c->id_empresa == 0){
			...
			$this->values = array($c->empresa_t,$fantasia,$tipo,$c->cpf,$c->rg,$c->nome,
				...
				null,null,null,utf8_decode('Implantação'),null,null,null,'0.00','0.00',null,null,null,
			...
			if(count($ret) > 0){
                            ...
                            if(count($ret2) > 0){
                                ...
                                foreach($ret2 as $d => $res){
                                    if(!is_dir("uploads/expansao/".$ret[0]->id_empresa)){
                                        mkdir("uploads/expansao/".$ret[0]->id_empresa, 0777); 
                                    } 
                                    $origem = "anexos_expansao/".$res->arquivo;
                                    $destino = "uploads/expansao/".$ret[0]->id_empresa."/".$res->arquivo;
                                    copy($origem, $destino);
                                    $this->sql = "INSERT INTO vsites_user_empresa_upload (id_empresa,id_usuario,ativo,arquivo,data) 
                                            VALUES (?,?,?,?,?)";
                                    $this->values = array($ret[0]->id_empresa,$controle_id_usuario,1,$destino,date('Y-m-d H:i:s'));
                                    $this->exec();
                                    $i++;
                                }
                            }
			...
			$html = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" 
				...
				<a href="http://www.cartoriopostal.com.br/sistemav2/franquias-editar.php?pagina=1&id='.$ret[0]->id_empresa.'" target="_blank">Clique aqui</a> 
			...
			if(PRODUCAO == 0){
                            ...
                            $From = 'Sistema Expansão';
                            $AddAddress = lista_emails('implantacao');
                            $AddCC = $c->user_email.','.$c->user_nome;
...
$SQL = "
	...
	public function forma_pagto2($acao = 1, $elemento = ''){
            $arr = array('8'=>'Agendamento','1'=>'E-mail','6'=>'Não Atende','7'=>'Reunião',
                '3'=>'Skype','2'=>'Telefone','4'=>'Telefone/E-mail','5'=>'Outros');
            switch($acao){
                case 1:
                    return $arr;
                    break;
                ...
                case 2:
                    if (array_key_exists($elemento, $arr)) {
                        return $arr[$elemento];
                    } 
                    return '';
                    break;
	...
	public function tipo_upload($acao, $id){
		$valor = array(0,4,7,3,1,2,8,5,6);
			$texto = array('','COF','Comprovante de Residência','Contrato',
                            'Cópia do CPF','Cópia do RG','Declaração do recebimento da COF',
				'Formulário de Adesão preenchido e assinado','Serasa');
		if($acao == 1){
                    $busca = array_search($id, $valor);
                    return utf8_encode(utf8_decode($texto[$busca]));
		} else {
	...
	public function listarfichaiddia(){
        ...
        public function list_form_del_anexo($id_ficha, $id_anexo, $id_usuario){
            $this->sql = "UPDATE site_ficha_anexos SET ativo = 0 WHERE id_ficha = ? AND id_anexo = ?";
            $this->values = array($id_ficha, $id_anexo);
            if($id_usuario != 1){
                $this->sql .= ' AND id_usuario = ?';
                $this->values[] = $id_usuario;
            }
            $this->exec();
        }
        
        public function list_form_edit($acao, $ci){
            global $controle_id_usuario;
            if($acao != 5){
                ...
                $list[0] = array('id_usuario','nome','rg','cpf','email','nascimento','tel_res','tel_rec',
                            'tel_cel','id_operadora','estado_civil','filhos','filhos_quant','endereco','numero',
                            'complemento','bairro','cep','estado','cidade','cargo','empresa_t','historico',
                            'escolaridade','cursos','formado','negocios','empresa_p','ramo_at','periodo',
                            'funcionarios','faturamento','capital','valor_disp','emprestimo','capital_terc',
                            'inicio_neg','dedicado_franq','fonte_renda','socios','caixa_empresa','conheceu_cp',
                            'unidades','unidades_valor','comunicados','interesse','estado_interesse','cidade_interesse',
                            'observacao','observacao_expansao','ficha_enviada','status','num_cof','cof_emitido',
                            'anexo','origem','data','data_impressao','id_user_alt','id_status','data_reuniao',
                            'data_reuniao_inclusao','data_cad_updt','data_cad_user_updt','id_empresa');

                $list[1] = array('nacionalidade', 'local_nascimento', 'regime', 'data_casamento', 'nome_pai',
                            'nome_mae', 'tip_imovel', 'reside_praca', 'franqueado', 'experiencia', 'motivo', 
                            'qual_franquia', 'opiniao', 'contato', 'faculdade', 'conclusao', 'orgao_emissor', 
                            'nome_socio', 'faturamento2', 'funcionarios2', 'profissao');

                $list[2] = array('endereco','numero','complemento','bairro','estado','cidade','cep');

                $list[3] = array('nome','rg','cpf','email','nascimento',
                            'nome_pai','nome_mae','profissao','cargo','empresa',
                            'telefone','admissao','end_empresa','numero',
                            'complemento','bairro','estado','cidade','cep');

                $list[4] = array('banco','cartao_credito','vencimento','limite','telefone','nome_gerente','agencia_conta');

                $list[5] = array('honorarios','salarios','comissoes','salario_conjuge','renda_alugueis',
                    'emprestimo_financeiro');

                $list[6] = array('modelo_veiculo','marca_veiculo','ano_veiculo','placa_veiculo','valor_veiculo',
                    'financiado','imovel','valor_venal','somatoria');

                $list[7] = array('tipo_franquia','num_cof','valor_cof','forma_pagto','origem','num_cof_emitida',
                    'valor_efetivo','valor_royaltie');

                $tabs = array('site_ficha_cadastro','site_ficha_cadastro_adicionais',
                    'site_ficha_cadastro_endereco2','site_ficha_cadastro_conjuge',
                    'site_ficha_cadastro_referencias_bancarias','site_ficha_cadastro_demonstrativo_rendimento',
                    'site_ficha_cadastro_bens_consumo','site_ficha_cadastro_dados_administrativos');

                $table = array($tabs[0],$tabs[1],$tabs[2],$tabs[3],$tabs[4],$tabs[5],
                    $tabs[6],$tabs[7]);
                $arr[] = $list[0];
                $arr[] = $list[1];
                $arr[] = $list[2];
                ...
                if($ci->id_ficha > 0){
                    for($j = 0; $j < count($table); $j++){
                        ...
                        if($j > 0){
                            $this->sql = "SELECT COUNT(*) AS total FROM ".$table[$j]." WHERE id_ficha = ?";
                            $this->values = array($ci->id_ficha);
                            $total = $this->fetch();
                            $total = $total[0]->total;
                            if($total == 0){
                                $this->sql = "INSERT INTO ".$table[$i]." (id_ficha) VALUES (?)";
                                $this->exec();
                            }
                        }
                        ...
                        $campos = array();
                        $token = array();                          
                        $this->sql = "UPDATE ".$table[$j]." SET ";
                        for($i = 0; $i < count($arr[$j]); $i++){
                            switch($j){
                                case 2: $nome = 'anterior_'.$arr[$j][$i]; break;
                                case 3: $nome = 'conjuge_'.$arr[$j][$i]; break;
                                case 6:
                                    $nome = ($arr[$j][$i] == 'telefone') ? $arr[$j][$i].'_banco' : $arr[$j][$i];
                                    break;
                                case 7:
                                    switch($arr[$j][$i]){
                        ...
                        if($j == 0){
                            $campos[] = 'data_cad_updt=?';
                            $campos[] = 'id_user_alt=?';
                            $this->values[] = date('Y-m-d');
                            $this->values[] = $controle_id_usuario;
                        }
                        
                        $this->sql .= implode(',', $campos);
                        $this->sql .= " WHERE id_ficha = ?";
                        $this->values[] = $ci->id_ficha;
                        #echo '<p>&nbsp;</p>'.$this->sql; 
                        #print_r($this->values);
                        if(count($campos) > 0){ $this->exec(); }
                    }
                    ...
                    if($acao == 4){
                        ...
                        if(is_numeric($err)){
                            $ext  = strtolower(end(explode('.', $_FILES['arquivo']['name'])));
                            $nome = $ci->id_ficha.date('YmdHis').'.'.$ext;
                            $err  = ExtensaoOk($ext,2);
                            if(is_numeric($err)){
                                if(move_uploaded_file($_FILES['arquivo']['tmp_name'], 'anexos_expansao/'.$nome)){
                                    $this->sql = "INSERT INTO site_ficha_anexos (id_ficha,id_usuario,tipo_upload,arquivo,nome,ativo) "
                                            . "VALUES (?,?,?,?,?,?)";
                                    $this->values = array($ci->id_ficha, $controle_id_usuario, $ci->tipo_upload,
                                        $nome, $_FILES['arquivo']['name'], 1);
                                    $this->exec();
                ...
                switch($ci->id_status){
                    case 5: case 10: case 19: case 12:
                        $ci->data_reuniao = verifica_invert_data($ci->data_reuniao);
                        if(!validaData2($ci->data_reuniao,'AAAA-MM-DD')){
                            $ci->data_reuniao = '0000-00-00';
                        }
                        $ci->data_reuniao = $ci->data_reuniao;
                        break;
                    
                    default: $ci->data_reuniao = '0000-00-00';
                }
                
                $this->sql = "INSERT INTO site_ficha_cadastro_historico (id_ficha,id_user_alt,
                    id_status,forma_pagto,data_reuniao,data_inclusao,observacao) "
                        . "VALUES (?,?,?,?,?,?,?)";
                $this->values = array($ci->id_ficha, $controle_id_usuario, $ci->id_status,
                    $ci->forma_pagto2, $ci->data_reuniao, date('Y-m-d H:i:s'), $ci->observacao_expansao);
                $this->exec();
                               
                $this->values = array();
                switch($ci->id_status){
                    case 19: case 21: 
                        $this->sql = "UPDATE site_ficha_cadastro SET data_cad_updt=?,"
                            . "id_user_alt=?, observacao_expansao=?, data_reuniao_inclusao=? ";
                        $this->values[] = date('Y-m-d');
                        $this->values[] = $controle_id_usuario;    
                        $this->values[] = $ci->observacao_expansao;
                    ...
                    default: 
                        $this->sql = "UPDATE site_ficha_cadastro SET data_cad_updt=?,"
                            . "id_user_alt=?, id_status=?, observacao_expansao=?, data_reuniao_inclusao=? ";
                        $this->values[] = date('Y-m-d');
                        $this->values[] = $controle_id_usuario;
                        $this->values[] = $ci->id_status;
                        $this->values[] =  $ci->observacao_expansao;
                        $this->values[] = date('Y-m-d H:i:s');
                        $this->sql .= "WHERE id_ficha = ?";
                        $this->values[] = $ci->id_ficha;
                ...
                $c = $this->list_forms($ci->id_ficha);
                $c = UTF_Encodes($c[0]); 
                $c->id_ficha = $ci->id_ficha;
                switch($ci->id_status){
                    case 4: 
                        $this->email_elaborar_cof($c); 
                        break;
        ...
        public function list_forms_historico_status($id){
            $this->sql = "SELECT * FROM site_ficha_cadastro_status WHERE id_status = ?";
            $this->values = array($id);
            return $this->fetch();
        }
                
        public function list_forms($id){
            $b = new stdClass();
            
            $tab = array('site_ficha_cadastro_conjuge','site_ficha_cadastro_adicionais',
                'site_ficha_cadastro_bens_consumo','site_ficha_cadastro_demonstrativo_rendimento',
                'site_ficha_cadastro_endereco2','site_ficha_cadastro','site_ficha_cadastro_referencias_bancarias',
                'site_ficha_cadastro_dados_administrativos');
            $arr = array();
            $r = array();
            
            #conjuge;
            $arr[] = array('nome','rg','cpf','email','nascimento',
                'nome_pai','nome_mae','profissao','cargo','empresa',
                'telefone','admissao','end_empresa','numero',
                'complemento','bairro','estado','cidade','cep');

            $arr[] = array('nacionalidade', 'local_nascimento', 'regime', 'data_casamento', 'nome_pai',
                'nome_mae', 'tip_imovel', 'reside_praca', 'franqueado', 'experiencia', 'motivo', 
                'qual_franquia', 'opiniao', 'contato', 'faculdade', 'conclusao', 'orgao_emissor', 
                'nome_socio', 'faturamento2', 'funcionarios2', 'profissao');
            
            $arr[] = array('modelo_veiculo','marca_veiculo','ano_veiculo','placa_veiculo','valor_veiculo',
                'financiado','imovel','valor_venal','somatoria');
            
            $arr[] = array('honorarios','salarios','comissoes','salario_conjuge','renda_alugueis',
                'emprestimo_financeiro');
            
            $arr[] = array('endereco','numero','complemento','bairro','estado','cidade','cep');
            
            $arr[] = array('id_usuario','nome','rg','cpf','email','nascimento','tel_res','tel_rec',
                'tel_cel','id_operadora','estado_civil','filhos','filhos_quant','endereco','numero',
                'complemento','bairro','cep','estado','cidade','cargo','empresa_t','historico',
                'escolaridade','cursos','formado','negocios','empresa_p','ramo_at','periodo',
                'funcionarios','faturamento','capital','valor_disp','emprestimo','capital_terc',
            ...
            $arr[] = array('banco','cartao_credito','vencimento','limite','telefone','nome_gerente','agencia_conta');

            $arr[] = array('tipo_franquia','num_cof','valor_cof','forma_pagto','origem','num_cof_emitida',
                'valor_efetivo','valor_royaltie');
            
            if(isset($id) AND is_numeric($id) AND $id > 0){
                for($i = 0; $i < count($tab); $i++){
                    $this->sql = "SELECT * FROM ".$tab[$i]." WHERE id_ficha = ?";
                    $this->values = array($id);
                    $ret = $this->fetch();
                    if(count($ret) > 0){
                        $ret = $ret[0];
                        ...
                        $this->sql = "INSERT INTO ".$tab[$i]." (id_ficha) VALUES (?)";
                        $this->values = array($id);
                        $this->exec();
                        $this->list_forms($id);
                        foreach($arr[$i] AS $f){
