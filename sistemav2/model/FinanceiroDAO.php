...
class FinanceiroDAO extends Database{
	...
	**/
	public function listaRoyIn($im){
		$this->sql = "select replace(ue.fantasia,'Cartório Postal - ','') as fantasia, rr.id_rel_royalties, rr.roy_rec, rr.fpp_rec, rr.valor_propaganda, rr.valor_royalties, rr.id_empresa
	...
	*/
	public function editarRecebimento($id_financeiro,$id_pedido_item,$controle_id_empresa,$departamento_p,$departamento_s,$f){
        ...
        public function buscaRecebimentoItem($id = 0){
            $this->sql = "select pi.id_empresa_atend, p.nome AS sacado, CONCAT(p.endereco,'',p.numero,' ',p.complemento) AS endereco,
                p.estado, p.cidade, p.bairro, p.cep,
                st.status,pi.id_fatura, pi.valor_rec as total, pi.certidao_estado, pi.certidao_cidade, pi.certidao_devedor, pi.data_prazo, pi.status_hora,
		...
		pi.id_pedido_item=pif.id_pedido_item";
	...
	public function buscaRecebimento($busca,$id_empresa,$pagina){
		...
		$url_busca = substr(str_replace('pagina='.$pagina.'&','',$url_busca),$url_busca_pos+5);
                ...
                $busca->busca_data_i = (isset($busca->busca_data_i) AND strlen($busca->busca_data_i) > 0) ? invert($busca->busca_data_i, '-', 'SQL') : '';
                $busca->busca_data_f = (isset($busca->busca_data_f) AND strlen($busca->busca_data_f) > 0) ? invert($busca->busca_data_f, '-', 'SQL') : '';
                $busca->busca_data_i_a = (isset($busca->busca_data_i_a) AND strlen($busca->busca_data_i_a) > 0) ? invert($busca->busca_data_i_a, '-', 'SQL') : '';
                $busca->busca_data_f_a = (isset($busca->busca_data_f_a) AND strlen($busca->busca_data_f_a) > 0) ? invert($busca->busca_data_f_a, '-', 'SQL') : '';
                $busca->busca_data_i_f = (isset($busca->busca_data_i_f) AND strlen($busca->busca_data_i_f) > 0) ? invert($busca->busca_data_i_f, '-', 'SQL') : '';
                $busca->busca_data_f_f = (isset($busca->busca_data_f_f) AND strlen($busca->busca_data_f_f) > 0) ? invert($busca->busca_data_f_f, '-', 'SQL') : '';
                $busca->busca_data_ci = (isset($busca->busca_data_ci) AND strlen($busca->busca_data_ci) > 0) ? invert($busca->busca_data_ci, '-', 'SQL') : '';
                $busca->busca_data_cf = (isset($busca->busca_data_cf) AND strlen($busca->busca_data_cf) > 0) ? invert($busca->busca_data_cf, '-', 'SQL') : '';
		...
		$busca->busca_ordenar_por=' pi.id_pedido '.(isset($busca->busca_ordenar_por_o)?$busca->busca_ordenar_por_o:'').', pi.ordem '.(isset($busca->busca_ordenar_por_o)?$busca->busca_ordenar_por_o:'');
		...
		if($busca->busca_id_status<>''){ $onde .= " and pi.id_status='".$busca->busca_id_status."'"; }
		...
		if($busca->busca_id_departamento<>''){ $onde .= " and pi.id_servico_departamento='".$busca->busca_id_departamento."'"; }
		if($busca->busca_id_atividade<>''){ $onde .= " and pi.id_atividade='".$busca->busca_id_atividade."'"; }
		if($busca->busca_origem<>''){      $onde .= " and p.origem='".$busca->busca_origem."'"; }
		if($busca->busca_forma_pagamento<>'') $onde .= " and p.forma_pagamento= '".$busca->busca_forma_pagamento."' ";
		if(isset($busca->busca_contato) AND $busca->busca_contato<>'') $onde .= " and p.contato like '".$busca->busca_contato."%' ";
		...
		if($busca->busca_data_i_f<>'')    { $onde .= " and pi.inicio>='".str_replace(' 00:00:00','',$busca->busca_data_i_f)." 00:00:00'"; }
		if($busca->busca_data_f_f<>'')    { $onde .= " and pi.inicio<='".str_replace(' 23:59:59','',$busca->busca_data_f_f)." 23:59:59'"; }
		if($busca->busca_data_i_a<>'')    { $onde .= " and pi.data_atividade>='".str_replace(' 00:00:00','',$busca->busca_data_i_a)." 00:00:00'"; }
		if($busca->busca_data_f_a<>'')    { $onde .= " and pi.data_atividade<='".str_replace(' 23:59:59','',$busca->busca_data_f_a)." 23:59:59'"; }
		if($busca->busca_data_i<>'')    { $onde .= " and pi.data>='".str_replace(' 00:00:00','',$busca->busca_data_i)." 00:00:00'"; }
		if($busca->busca_data_f<>'')    { $onde .= " and pi.data<='".str_replace(' 23:59:59','',$busca->busca_data_f)." 23:59:59'"; }
		if($busca->busca_data_ci<>'')    { $onde .= " and pi.encerramento>='".str_replace(' 00:00:00','',$busca->busca_data_ci)." 00:00:00'"; }
		if($busca->busca_data_cf<>'')    { $onde .= " and pi.encerramento<='".str_replace(' 23:59:59','',$busca->busca_data_cf)." 23:59:59'"; }
		if($busca->busca<>''){ $onde .= " and (p.nome like '".$busca->busca."%' or pi.certidao_devedor = '".$busca->busca."' or pi.certidao_nome like '".$busca->busca."%' or pi.certidao_pai like '".$busca->busca."%' or pi.certidao_mae like '".$busca->busca."%' or pi.certidao_esposa like '".$busca->busca."%' or pi.certidao_marido like '".$busca->busca."%' or replace(replace(replace(pi.certidao_cpf,'.',''),'-',''),'/','') = replace(replace(replace('".$busca->busca."','.',''),'-',''),'/','') or replace(replace(replace(pi.certidao_cnpj,'.',''),'-',''),'/','') = replace(replace(replace('".$busca->busca."','.',''),'-',''),'/','') or replace(replace(replace(p.cpf,'.',''),'-',''),'/','') = replace(replace(replace('".$busca->busca."','.',''),'-',''),'/','')) "; }
	...
	public function atualizaConta($ids, $conta, $id_empresa){
        ...
        public function atualizaConta2($ids, $conta, $id_empresa){
		$this->sql = 'UPDATE vsites_financeiro SET financeiro_nossa_conta=? WHERE id_financeiro IN (?) and id_empresa_fin=?';
		$this->values=array($conta,implode(',',$ids),$id_empresa);
		return $this->exec();
	}
	...
	**/
	...
	public function buscaDesembolso($busca,$id_empresa,$pagina){
		...
		if($busca->busca_id_pedido<>''){   $onde .= " and pi.id_pedido='".$busca->busca_id_pedido."'"; }
		if($busca->busca_ordem<>''){   $onde .= " and pi.ordem='".$busca->busca_ordem."'"; }

		if($busca->busca_id_departamento<>''){
			$onde .= " and pi.id_servico_departamento='".$busca->busca_id_departamento."' ";
		}

		if($busca->busca_nossa_conta<>''){   $onde .= " and trim(f.financeiro_nossa_conta)=trim('".$busca->busca_nossa_conta."')"; }
		...
		if($busca->busca_data_i<>'' and $busca->busca_autorizacao!=0){ $onde .= " and f.financeiro_autorizacao_data>='".$busca->busca_data_i."'"; }
		if($busca->busca_data_i<>'' and $busca->busca_autorizacao==0){ $onde .= " and f.financeiro_data>='".$busca->busca_data_i."'"; }

		if($busca->busca_data_f<>'' and $busca->busca_autorizacao!=0){ $onde .= " and f.financeiro_autorizacao_data<='".$busca->busca_data_f."'"; }
		if($busca->busca_data_f<>'' and $busca->busca_autorizacao==0){ $onde .= " and f.financeiro_data<='".$busca->busca_data_f."'"; }
	...
	**/
	...
	public function buscaRecebimentoF($busca,$id_empresa,$pagina){
		...
		if($busca->busca_ordenar=='Serviço') 		$busca->busca_ordenar_por=' servico '; else
		...
		if($busca->busca_data_i<>'')    { $onde .= " and f.financeiro_data>='".$busca->busca_data_i." 00:00:00'"; }
		if($busca->busca_data_f<>'')    { $onde .= " and f.financeiro_data<='".$busca->busca_data_f." 23:59:59'"; }

		if($busca->busca_id_pedido<>'') $onde .= " and pi.id_pedido= '".$busca->busca_id_pedido."' ";
		if($busca->busca_id_empresa<>''){ $onde .= " and pi.id_empresa_atend='".$busca->busca_id_empresa."'"; }
		
		if($busca->busca_id_status=='Todos') $busca->busca_id_status='';
		if($busca->busca_id_status<>''){ $onde .= " and pi.id_status='".$busca->busca_id_status."'"; } 
		else $onde .= " and pi.id_status!='14' and pi.id_status!='10'";
		if($busca->busca_id_status==8){ $onde .= " and pi.data_status='0000-00-00 00:00:00'"; } 
		if($busca->busca_id_departamento<>''){ $onde .= " and pi.id_servico_departamento='".$busca->busca_id_departamento."'"; }
		...
		$this->sql = "SELECT ".$campo." ".$condicao;
                if(!defined('NO_PAGINACAO')){
	...
	**/
    ...
    function desembolso_view($pedido_item, $id_empresa){
        $this->sql = "SELECT f.*, uu.nome FROM vsites_financeiro as f, vsites_user_usuario as uu "
                . "where f.id_pedido_item=? and f.financeiro_tipo='Desembolso' and "
                . "f.id_usuario=uu.id_usuario and uu.id_empresa=?";
        $this->values = array($pedido_item, $id_empresa);
        return $this->fetch();
    }
    
    function desembolso_form($id_pedido_item, $id_financeiro){
        $this->sql = "SELECT f.*, uu.nome FROM vsites_financeiro as f, vsites_user_usuario as uu where f.id_pedido_item=? and f.id_financeiro=? and f.financeiro_tipo='Desembolso' and f.id_usuario=uu.id_usuario";
        $this->values = array($id_pedido_item, $id_financeiro);
        return $this->fetch();
    }
    
    function classificacao(){
        $this->sql = "SELECT * from vsites_classificacao  where ordem='1' order by classificacao";
        return $this->fetch();
    }
    
    function arquivo($controle_id_empresa, $onde, $acao = 1){
        switch($acao){
            case 1:
        
                $this->sql = "SELECT f.financeiro_forma, f.rel, f.financeiro_valor+f.financeiro_sedex+f.financeiro_rateio as financeiro_valor, f.financeiro_desembolsado, f.financeiro_descricao, f.financeiro_troco, f.financeiro_autorizacao_data, f.financeiro_agencia, f.financeiro_conta, f.financeiro_favorecido, f.financeiro_identificacao, u.nome, pi.id_pedido, pi.ordem from
            ...
            case 2:
                $this->sql = "update vsites_user_usuario as u, vsites_pedido_item as pi, vsites_financeiro as f 
                    set f.rel='1'	where
                u.id_empresa = '".$controle_id_empresa."' and
                    f.financeiro_tipo='Desembolso' and
                f.id_usuario = u.id_usuario and
    ...
    public function RoyEmpresa($id_royaltie = 0){
        $this->sql = "select date_format(rr.data,'%m/%Y') as ref, replace(ue.fantasia,'Cartório Postal - ','') as fantasia, rr.id_rel_royalties, rr.roy_rec, rr.fpp_rec, rr.valor_propaganda, rr.valor_royalties, rr.id_empresa
        from vsites_user_empresa as ue, vsites_rel_royalties as rr where rr.id_rel_royalties = ? and ue.id_empresa=rr.id_empresa ORDER BY fantasia, ref";
        $this->values = array($id_royaltie);
        return $this->fetch();
    }   
    
    public function exportar_arquivo($campo, $condicao,$acao = 1){
        switch($acao){
            case 1:
                $sql = "select ".$campo." ".$condicao." limit 1300";
                break;
            case 2:
                global $data_prazo_inc, $sql_franquia, $sql_usuario_resp,$id_pedido_item,
                    $controle_id_empresa, $busca_ordenar_por;
                $sql = "SELECT ".$data_prazo_inc." as data_prazo,(".$sql_franquia.") as fantasia,(".$sql_usuario_resp.") as responsavel, pi.certidao_matricula, pi.certidao_cidade, pi.certidao_estado, pi.id_empresa_resp,
