...
class FinanceiroVerificaDAO extends Database{
	...
	public function inserir($id_pedido_item,$id_empresa,$departamento_p,$departamento_s,$f){
		...
		if($cont[0]->inicio=='0000-00-00 00:00:00' or $cont[0]->inicio==''){
			$p_valor->error .= '<li><b>Antes de solicitar desembolso, aplique a atividade "Serviço Conferido" e depois "Iniciar Serviço" consecutivamente. Caso o pedido esteja com forma de pagamento em dinheiro é preciso que o financeiro acuse o recebimento do valor no menu Financeiro > Recebimento</b></li>';
			return $p_valor;
		}
		...
		if($f->financeiro_classificacao == '' || $f->financeiro_descricao == '' || ($f->financeiro_valor=='' && $f->financeiro_sedex=='' && $f->financeiro_rateio=='') || $f->financeiro_forma == ''){
		...
		if(!is_numeric($f->financeiro_rateio) and $f->financeiro_rateio<>''){
			...
			$p_valor->error .= '<li>O campo "Honorários" não é válido</li>';
		}

		if(!is_numeric($f->financeiro_sedex) and $f->financeiro_sedex<>''){
			$p_valor->financeiro_sedex = 1;
			$p_valor->error .= '<li>O campo "Correio" não é válido</li>';
		}

		if(!is_numeric($f->financeiro_valor) and $f->financeiro_valor<>''){
			$p_valor->financeiro_valor = 1;
			$p_valor->error .= '<li>O campo "Custas" não é válido</li>';
		}
	...
	public function editar($id_pedido_item,$id_financeiro,$id_empresa,$departamento_p,$departamento_s,$f){
		...
		$data = gmdate('Y-m-d',strtotime('-12 month',strtotime(date('Y-m-d'))));
		$this->sql = "SELECT COUNT(*) as total, pi.operacional, pi.id_empresa_resp from vsites_financeiro as f, vsites_pedido_item as pi where 
		f.id_financeiro=? and
		...
		$this->values = array($id_financeiro,$id_empresa,$data);
		$cont = $this->fetch();
		...
		if($f->financeiro_classificacao == '' or $f->financeiro_descricao == '' or $f->financeiro_valor == '' or $f->financeiro_forma == ''){
			$p_valor->error .= '<li>Os campos com * são obrigatórios</li>';
	...
	public function verificaDeletaReceb($id_financeiro,$id_empresa,$departamento_p,$departamento_s){
		$p_valor = new stdClass();
		if(in_array('2', $departamento_p)!=1 and in_array('1', $departamento_s)!=1){
		...
		if($id_empresa!=1) $data = gmdate('Y-m-d',strtotime('-1 month',strtotime(date('Y-m-d'))));
		else $data = gmdate('Y-m-d',strtotime('-24 month',strtotime(date('Y-m-d'))));
		$this->sql = "select COUNT(0) as total, id_pedido_item, financeiro_valor from vsites_financeiro as f where f.id_financeiro=? and f.financeiro_tipo='Recebimento' and date_format(financeiro_data,'%Y-%m-%d')>=? and f.id_empresa_fin=? group by f.id_pedido_item";
		$this->values = array($id_financeiro,$data,$id_empresa);
		$cont = $this->fetch();

		if($cont[0]->total==0 and $id_empresa!=1){
	...
	public function verificaEditaReceb($id_financeiro,$id_pedido_item,$id_empresa,$departamento_p,$departamento_s,$f){
		...
		$this->sql = "SELECT COUNT(0) as total from vsites_financeiro as f where f.id_financeiro=? and f.financeiro_tipo='Recebimento' and date_format(f.financeiro_data,'%Y-%m-%d')>=? and f.id_empresa_fin=?";
		$this->values = array($id_financeiro,$data,$id_empresa);
		$cont = $this->fetch();
	...
	/**
	 * verifica se pode pode recusar o pedido
	 */
	public function verificaRecusaPedido($id_financeiro,$id_pedido_item,$id_empresa){

		$this->sql = "select pi.id_pedido, pi.ordem, pi.id_empresa_resp, pi.id_pedido_item, pi.id_usuario_op, pi.operacional, pi.id_usuario_op2 from 
		vsites_financeiro as f, vsites_pedido_item as pi where
		f.id_financeiro = ? and
		f.id_empresa_fin!=? and 
		...
		pi.id_empresa_resp=? limit 1";
		$this->values = array($id_financeiro,$id_empresa,$id_empresa);
		$cont = $this->fetch();
		return $cont[0];
	 ...
	 * verifica se pode pode recusar o pedido
	 */
	public function verificaAprovaPedido($id_financeiro,$id_empresa){
		$sql_rec = "select SUM(ff.financeiro_valor_f) as valor from vsites_financeiro_f as ff where 
		ff.id_financeiro=f.id_financeiro and ff.financeiro_autorizacao='Aprovado' and ff.financeiro_tipo='Recebimento' group by ff.id_financeiro";
		
		$this->sql = "select f.id_pedido_item, (".$sql_rec.") as valor_rec, (financeiro_valor+f.financeiro_sedex+f.financeiro_rateio) as financeiro_valor, 
				pi.id_status, pi.data_prazo, pi.dias, pi.id_pedido, pi.ordem
				from vsites_financeiro as f, vsites_pedido_item pi 
				where
				f.id_financeiro  = ? and
				f.id_empresa_fin!= ? and
				f.financeiro_classificacao = '38' and
				f.financeiro_tipo = 'Desembolso' and
				f.financeiro_autorizacao = 'Aprovado' and
				f.id_pedido_item = pi.id_pedido_item limit 1";
		$this->values = array($id_financeiro,$id_empresa);
		$cont = $this->fetch();
		return count($cont) > 0 ? $cont[0] : array();
	 ...
	 * verifica se pode pode recusar o pedido
	 */
	public function verificaAprovaPedidoLista($id_financeiro,$id_empresa){
	
		$sql_rec = "select SUM(ff.financeiro_valor_f) as valor from vsites_financeiro_f as ff where ff.id_financeiro=f.id_financeiro and ff.financeiro_autorizacao='Aprovado' and ff.financeiro_tipo='Recebimento' group by ff.id_financeiro";

		$this->sql = "SELECT (".$sql_rec.") as valor_rec, (f.financeiro_valor+f.financeiro_sedex+f.financeiro_rateio) AS financeiro_valor, f.id_financeiro
		from vsites_financeiro as f where
		f.id_financeiro = ? and
		f.id_empresa_fin != ? and
		f.financeiro_classificacao = '38' and
		f.financeiro_tipo = 'Desembolso' and
