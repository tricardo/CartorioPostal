...
class PagamentoDAO extends Database{
	...
	public function inserir($p){
		...
		$p->pis = isset($p->pis) ? number_format($p->pis,2,".","") : '0.00';
		$p->ir = isset($p->ir) ? number_format($p->ir,2,".","") : '0.00';
		$p->cofins = isset($p->cofins) ? number_format($p->cofins,2,".","") : '0.00';
                $p->conta = isset($p->conta) ? $p->conta : '';
                $p->id_conta = isset($p->id_conta) ? $p->id_conta : 0;
                $p->id_compra = isset($p->id_compra) ? $p->id_compra : '';
                $p->qt_parcelas = isset($p->qt_parcelas) ? $p->qt_parcelas : '';
                $p->fisico = isset($p->fisico) ? 1 : '';


		if(isset($p->qt_parcelas) AND $p->qt_parcelas<>'' and $p->qt_parcelas!=0){
