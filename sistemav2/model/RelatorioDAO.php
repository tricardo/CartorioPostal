...
class RelatorioDAO extends Database{
	...
	public function busca($id_empresa, $mes, $ano, $tipo, $pagina=1){
            global $c;
            $this->values = array();
            $this->link = 'mes='.$mes.'&ano='.$ano.'&rel='.$c->rel.'&id_empresa='.$id_empresa.'&pg='.$c->pg;

            $where = " WHERE data_relatorio LIKE ? AND descricao LIKE ?";
            $this->values[]=$ano.'-'.$mes.'%';
            $this->values[]= utf8_decode($tipo).'%';
            if($id_empresa!=''){
