...
class ServicoDAO extends Database{
	...
	/**
	 * retorna uma lista de servi�os, por empresa
	 * departament,alguma string para busca e p�gina
	 *
	...
	public function busca($controle_id_empresa,$id_departamento='', $busca='',$id_servico='', $pagina=1){
		...
		if($id_servico!='' AND $id_servico > 0){
	...
	public function carregarCombo(){
        ...
        public function remessa(){
            $this->sql = "SELECT id_servico, descricao from vsites_servico as s where s.status='Ativo' and id_servico='17' order by descricao";
            return $this->fetch();
        }
        
        public function editarServico($controle_id_empresa, $valor, $dias, $id_servico_var){
            $this->sql = "update vsites_servico_var set valor_".$controle_id_empresa."='".$valor."', dias_".$controle_id_empresa."='".$dias."' where id_servico_var='" . $id_servico_var . "'";
            return $this->exec();
        }
}
