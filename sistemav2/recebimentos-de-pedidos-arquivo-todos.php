...
$linha_arq = utf8_decode('#'.$r_inicio.$r_prazo.$r_conclu.$r_data_atividade.';Solicitante;CPF;CNPJ'.$r_nome.$r_devedor.$r_forma.$r_cpf.$r_atendimento.$r_responsavel.$r_servico.$r_cidade.$r_estado.$r_atividade.$r_status.$r_agenda.';Valor'.';Custas'.';Honorários'.';Correios'.$r_departamento.';Controle do Cliente');
...
foreach($financeiroDAO->exportar_arquivo($campo, $condicao) AS $res){
	...
	if($busca_e_conclu==1) $r_conclu=';'.invert($res["operacional"],'/','php'); else $r_conclu='';
